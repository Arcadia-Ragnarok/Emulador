	...
	if (BaseJob != Job_Novice) {
		...
		if (!swordq) {
			if (getskilllv ("NV_BASIC") < 9) {
				...
				mes "Me pergunto o que faz aqui se nem suas habilidades básicas você aprendeu.";
				close;
			} else {
				mes "Um jovem Aprendiz!?";
				mes "Por acaso você veio até aqui para se juntar a nossa guilda se tornando um Espadachim?";
				next;
				mes "[Líder dos Espadachins]";
				...
				switch(select("Preencher o formulário","Perguntar sobre o Teste","Não fazer nada")) {
					...
					case 2:
					mes "[Líder dos Espadachins]";
					mes "Ah, sim, o teste de graduação........";
					next;
					mes "[Líder dos Espadachins]";
					mes "Bem, para ser espadachim a pessoa deve ser astuta e muito resitente as adversividades.";
					mes "E nosso teste apenas mede esse potencial de cada cadidato.";
					next;
					mes "[Líder dos Espadachins]";
					mes "Básicamente é só atravessar 3 partes de um campo, nele haverá armadilhas e monstros para dificultar.";
					mes "Alguns podem até pensar que é difícil, mas é muito simples.";
					mes "Com um pouco de perceverância que um espadachim deve ter, qualquer um pode conseguir.";
					close;
					case 3:
					mes "[Líder dos Espadachins]";
					mes "Muito bem, mas se intereçar volte aqui.";
					mes "Quem sabe você não se torna um espadachim.";
					close;
				}
			}
		} else if (swordq == 1) {
			mes "O que faz aqui?";
			mes "Vá fazer o seu teste";
			next;
			...
			mes "Se você se tornar Espadachim agora.";
			...
			mes "Leve o tempo que precisar, mesmo assim nós aqui já consideramos você como membro de nossa guilda.";
			close;
		}
	}
}
...
izlude_in,62,170,6	script	Espadachim#swordq	4_M_03,{
	...
	if (BaseJob != Job_Novice) {
		...
		mes "Faça sua inscrição com o lider da guilda bem alí á frente.";
		close;
	} else if (swordq == 1) {
		mes "Hunn...";
		...
		mes "Fale com o líder da guilda.";
		close;
	}
}
