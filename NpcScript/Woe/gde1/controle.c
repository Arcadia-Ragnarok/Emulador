...
-	script	Gld_Agit_Manager::Gld_Agit_Manager	FAKE_NPC,{
	...
	// --------------------------------------------------------------
	OnAgitBreak:
	.@GID = getcharid(CHARID_GUILD);
	// Mostra um Log de erro caso algum personagem sem clã quebre o emperium, mesmo que isso é impossível de acontecer
	if (.@GID <= 0) {
		.@notice$ = "O personagem "+strcharinfo(CHAR_NAME)+" ("+getcharid(CHARID)+") acaba de quebrar o Emperium no Castelo: "+strnpcinfo(NPC_NAME_HIDDEN)+" sem estar em um Clã. O Emperium será restaurado";
		logmes(.@notice$);
		debugmes(.@notice$);
