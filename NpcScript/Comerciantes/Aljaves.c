	...
	if (countitem(.@arrowItem) >= 500) {
		...
		switch(select("Trocar quantas Aljaves possíveis", "1 Aljave", "Cancelar")) {
			...
			if (Zeny > 500) {
				...
				mes "Me desculpe.";
				mes "Eu não tinha idéia que você era tão...";
		...
		mes "[Inventor Jaax]";
		mes "Você pode carregar no máximo 500 Flechas dentro deste Aljave.";
		mes "Isso foi feito usando meu método secreto, para que o peso total de Flechas e Aljaves juntos é menor do que se carregasse apenas Flechas.";
		next;
		mes "[Inventor Jaax]";
