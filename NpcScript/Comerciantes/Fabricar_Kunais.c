...
que_ng,72,29,3	script	Mercador de Kunais#KunaiTrade	4_M_01,{
	if (BaseClass != Job_Ninja) {
		mes("[Kashin]\n Como você encontrou esse lugar?!\n É melhor que saia o quanto antes, para o seu próprio bem.");
		close;
	} else {
		mes("[Kashin]\n Meu nome é Kashin e negocio de Kunais por itens.\n Veja se exite algo que lhe interece.");
		next;
		...
		mes("[Kashin]\n Você pode trocar " + .@shurAmmout + " " + getitemname(.@Shuriken) + " e " + .@itemAmmout + " " + getitemname(.@itemRequire) + " por 10 " + getitemname(.@KunaiItem) + ".");
		next;
		mes("[Kashin]\n Eu posso só lhe dar de cada vez um máximo de 500 Kunais.\n Se quiser cancelar a troca basta que digite 0.\n Quantas Kunais você gostaria?");
		next;
		input (.@kunaiAmmout);
		if (.@kunaiAmmout == 0) {
			mes("[Kashin]\n Você cancelou a negociação.\n Tudo bem volte quando quiser fazer negócio.");
			close;
		} else if (.@kunaiAmmout > 500) {
			mes("[Kashin]\n Parece que não me ouviu...\n Eu disse que só posso negociar 500 Kunais por vez.");
			close;
		} else if(countitem(.@Shuriken) < .@kunaiAmmout * .@shurAmmout || countitem(.@itemRequire) < .@kunaiAmmout * .@itemAmmout) {
			mes("[Kashin]\n Parece que você não tem a quantidade de itens necessários para a troca.");
			close;
		} else if(checkweight(.@KunaiItem, .@kunaiAmmout * 10) == 0) {
			mes("[Kashin]\n Tudo certo, mas temos um problema...\n Você está levando muito peso com você e não vai poder carregar as Kunais que eu lhe entregar.\n Livre-se de um pouco de peso no seu inventário e tente novamente.");
			close;
		} else {
			mes("[Kashin]\n Ótimo, tudo está em ordem, assim prossigamos com a troca.\n Só um instante....");
			next;
			mes("[Kashin]\n .....\n ......");
			next;
			mes("[Kashin]\n Espero que faça bom proveito com suas novas Kuanis.");
