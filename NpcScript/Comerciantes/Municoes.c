...
que_ng,187,156,3	script	Negociador de Munições#ammo	4_M_04,{
	if (BaseJob != Job_Gunslinger) {
		...
		mes("[Tony]\n Certo...\n Então lhe darei 30 unidades de " + getitemname(.@selectitem) + ".\n Em troca você me dar 1 Emveretarcon, 1 Fracon e " + .@requireunit + " " + getitemname(.@requirement) + ", estamos combinados?");
		next;
		if (select("Sim", "Não") == 1) {
...
que_ng,187,149,3	script	Caixas de Munição::mdk	4_M_01,{
	...
	next;
	mes("[Kenny]\n Eu posso colocar suas munições em carregadores, para que possa levar muito mais projéteis consigo.\n Então me diz no que está intereçad" + (Sex == SEX_MALE ? "o" : "a") + "?");
	next;
	switch(select("Pacote de Esferas de Relâmpago", "Pacote de Esferas Cegantes", "Pacote de Esferas Venenosas", "Pacote de Esferas Congelantes", "Pacote de Esferas de Chamas", "Carregador", "Carregador Sanguinário", "Carregador de Prata", "Cancelar")) {
		...
		case 9:
		mes("[Kenny]\n Sem problemas, não deixe de me procurar quando quiser guardar, seus projetéis em carregadores.");
		close;
	}
	mes("[Kenny]");
	if (countitem(.@bulletItem) >= 500) {
		mes(" Muito bem, eu posso colocar 500 " + getitemname(.@bulletItem) + " em um " + getitemname(.@bulletCase) + ".\n Para isso irei precisar dos projetéis e mais 500 zenys por cada carregador, essa é minha taxa de serviço.");
		next;
		switch(select("Me der um Carregador", "Me der quantos puder")) {
			...
			if (Zeny < 500) {
				...
				next;
				mes("[Kenny]\n Fique a vontade em me procurar, sempre quando precisar guardar seus projetéis em carregadores.");
				close;
			}
		}
	} else {
