		...
		while(true) {
			switch(select("Metalling", "Mineral", "Mineiros", "Fornalha Velha", "Terminar Conversa")) {
				...
				switch(select("...Isso seria?", "H-horripilante!")) {
					...
					next;
					mes "[Shena]";
					mes "Ao contrário dos novos dispositivos, as Fornalhas Velhas foram feitas por Mestres-Ferreiros que, eu acho, criaram as almas delas também.";
...
einbech,148,246,5	script	Franz#ein	4_M_REPAIR,{
	mes "[Franz]";
	mes "Tão entediante, morrer de fome para conversar.";
	mes "A-alguém...";
	next;
	mes "[Franz]";
	mes "Hei, um viajante!";
	...
	switch(select("Certo, tudo bem", "Não, obrigado")) {
		...
		switch(select("Criatura da Mina", "Incidente da Cidade")) {
			...
			next;
			mes "[Franz]";
			mes "De acordo com os aventureiros que enfrentaram esses ursos, eles são monstros pequenos, de propriedade neutra que deixam Mel, Prego Enferrujado, Biscoito de Gengibre e Martelo de Oridecon.";
			next;
			mes "[Franz]";
