...
gef_fild10,242,202,0	script	Memorial dos Orcs	2_MONEMUS,{
	...
	if (.@orctime == 1) {
		...
		if (getcharid(CHARID) == getpartyleader(.@party_id,2)) {
				...
				if (.@instance) {
					for (.@i = 1; .@i <= 2; ++.@i) {
						if (instance_attachmap(.@i + "@orcs", .@instance) == "") {
							...
							mesf("Líder do Grupo: %s", strcharinfo(CHAR_NAME));
...
2@orcs,26,164,0	script	Tocha#1-1	CLEAR_NPC,{
	if (getpartyleader(getcharid(CHARID_PARTY),2) != getcharid(CHARID)) end;
