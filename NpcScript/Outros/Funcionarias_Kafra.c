...
|                                                                   |
+-------------------------------------------------------------------+
| - Copyright: Spell Master (27/03/2017)                            |
| - Info: Funcionárias da Corporação Kafra.                         |
| - Note: Variável "merchantq","merchantq_2" e "merchantq_3"        |
|         pertencem a quest de mudança de classe para mercador.     |
\*-----------------------------------------------------------------*/
...
-	script	KafraCorp	FAKE_NPC,{
	...
	if (strnpcinfo(NPC_MAP) == "cmd_fild07")        { .@kaframenu = 1; }
	...
	switch (.@kafraoption) {
		...
		if (strnpcinfo(NPC_NAME_UNIQUE) == "comkaf2") { savepoint(strnpcinfo(NPC_MAP),127,134); }
		close2; cutin("",255); end;
		case 2: // Usar Armazém
		...
		if (strnpcinfo(NPC_MAP) == "prontera") {
			...
			switch (select("Prontera 1.200 Zenys","Payon 1.200 Zenys","Alberta 1.800 Zenys","Comodo 1.800 Zenys","Farol de Pharos 1.800 Zenys","Cancelar")) {
				...
				case 5: .@price = 1800; .@map$ = "cmd_fild07"; .@cordX = 127; .@cordY = 134; break;
				case 6:
				mes "[Funcionária Kafra]";
				mes "A Corporação Kafra estará sempre ao seu dispor para servir-l"+(Sex == SEX_MALE ? "o":"a")+".";
				close2; cutin("",255); end;
			...
			switch (select("Morroc 1.800 Zenys","Umbala 1.800 Zenys","Farol de Pharos 1.800 Zenys","Cancelar")) {
				case 1: .@price = 1800; .@map$ = "morocc";     .@cordX = 156; .@cordY = 46;  break;
				case 2: .@price = 1800; .@map$ = "umbala";     .@cordX = 100; .@cordY = 154; break;
				case 3: .@price = 1800; .@map$ = "cmd_fild07"; .@cordX = 127; .@cordY = 134; break;
				case 4:
				mes "[Funcionária Kafra]";
				mes "A Corporação Kafra estará sempre ao seu dispor para servir-l"+(Sex == SEX_MALE ? "o":"a")+".";
				close2; cutin("",255); end;
			...
			switch (select("Morroc 1.800 Zenys","Comodo 1.800 Zenys","Cancelar")) {
				case 1: .@price = 1800; .@map$ = "morocc"; .@cordX = 156; .@cordY = 46; break;
				case 2: .@price = 1800; .@map$ = "comodo"; .@cordX = 209; .@cordY = 143; break;
				case 3:
				mes "[Funcionária Kafra]";
				mes "A Corporação Kafra estará sempre ao seu dispor para servir-l"+(Sex == SEX_MALE ? "o":"a")+".";
				close2; cutin("",255); end;
