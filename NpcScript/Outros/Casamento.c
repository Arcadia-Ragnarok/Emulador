...
prt_church,100,128,4	script	Bispo#w	1_M_PASTOR,{
	...
	if (!getpartnerid()) {
		if (!$wedding) {
			if (wedding_sign == 1) {
				getpartymember(getcharid(CHARID_PARTY));
					...
					if (Sex == SEX_FEMALE) {
						if (strcharinfo(CHAR_NAME) == $wed_bride$) {
							...
							if (select("Sim, Eu caso.","^FF0000Não.^000000") == 1) {
								if (isloggedin(getcharid(CHARID_ACCOUNT,$wed_groom$))) {
									if (marriage($wed_groom$)) {
										wedding;
										sc_start(SC_WEDDING,3600000,1);
										getitem(Bride_Ring,1);
										attachrid(getcharid(CHARID_ACCOUNT,$wed_groom$));
										sc_start(SC_WEDDING,3600000,1);
										getitem(Bridegroom_Ring,1);
										detachrid;
										attachrid(getcharid(CHARID_ACCOUNT,$wed_bride$));
