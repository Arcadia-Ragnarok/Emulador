...
#ifdef NOISY
	ShowInfo("Guild salva ("CL_BOLD"%d"CL_RESET" - flag 0x%x).\n",g->guild_id, flag);
#endif
...
int inter_guild_calcinfo(struct guild *g) {
	...
	if(g->max_member > MAX_GUILD) {
		ShowError("Guild %d:%s tem capacidade para muitos membros (%d), max suportado: %d\n", g->guild_id, g->name, g->max_member, MAX_GUILD);
		g->max_member = MAX_GUILD;
	}
...
bool inter_guild_update_basic_info(int guild_id, int type, const void *data, int len) {
	...
	switch (type) {
		case GBI_EXP:
			...
			if( value < 0 && abs(value) > g->exp ) { return 0; }
			g->exp += value;
...
bool inter_guild_update_castle_data(int castle_id, int index, int value)
{
	...
	if (gc == NULL) {
		ShowError("inter_guild->update_castle_data: castelo id=%d nao encontrado\n", castle_id);
		return 0;
	}
	...
	switch (index) {
		...
		default:
			...
			ShowError("inter_guild->update_castle_data: Nao econtrado indice: %d\n", index);
			return 0;
	}
