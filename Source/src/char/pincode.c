...
bool pincode_config_read(const char *filename, const struct config_t *config)
{
	...
	if (libconfig->setting_lookup_bool_real(setting, "check_blacklisted", &pincode->check_blacklist) == CONFIG_FALSE) {
		ShowWarning("pincode 'check_blaclisted' nao encontrado, definindo para false...\n");
	...
	if (pincode->check_blacklist) {
		if ((temp = libconfig->setting_get_member(setting, "blacklist")) != NULL) {
			...
			for (i = 0; i < size; i++) {
				...
				if (strlen(pin) != 4) {
					ShowError("Tamnho inesperado %d de blacklist. Valor máximo = 4, recebido = %d\n", i, (int)strlen(pin));
					continue;
				}
				...
				if (j < VECTOR_LENGTH(duplicate)) {
					ShowWarning("Entrada duplicada no blacklist. #%d\n", i);
					continue;
				}
