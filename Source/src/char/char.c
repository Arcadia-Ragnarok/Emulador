...
int char_parse_fromlogin_connection_state(int fd)
{
	if (RFIFOB(fd,2)) {
		...
		ShowError("The communication passwords are set in Config/Servers/Map-Server.cs and Config/Servers/Char-Server.cs\n");
		sockt->eof(fd);
		return 1;
	} else {
...
int do_init(int argc, char **argv) {
	...
	chr->SQL_CONF_NAME = aStrdup("Config/Common/Inter-Server.cs");
	chr->INTER_CONF_NAME = aStrdup("Config/Common/Inter-Server.cs");
...
#define CHECK_OLD_LOCAL_CONF(oldname, newname) do { \
	if (stat((oldname), &fileinfo) == 0 && fileinfo.st_size > 0) { \
		...
		CHECK_OLD_LOCAL_CONF("conf/import/Char_Serve.txt", "conf/import/Char-Server.conf");
...
#undef CHECK_OLD_LOCAL_CONF
	...
	if (strcmp(chr->userid, "s1")==0 && strcmp(chr->passwd, "p1")==0) {
		...
		ShowNotice("And then change the user/password to use in Config/Servers/Char-Server.cs (or Conf/import/Char-Server.conf)\n");
	}
#endif
