...
/**
 * Carregar aquivo de database para PET
 */
void pet_read_db(void) {
	const char *config_filename = "Summon_DB/PetList.conf";
	int i = 0;
	...
	pet->read_db_libconfig(config_filename, i > 0 ? true : false);
}
...
int pet_read_db_libconfig(const char *filename, bool ignore_missing) {
	...
	if (!exists(filepath)) {
		if (!ignore_missing) {
			ShowError("pet_read_db_libconfig: Arquivo nao encontrado %s\n", filepath);
		}
		return 0;
	}
	...
	while ((t = libconfig->setting_get_elem(pdb, i++))) {
		...
		if (duplicate[pet_id]) {
			ShowWarning("pet_read_db_libconfig:%s: ID de pet duplicado #%d\n", filename, pet_id);
		} else {
...
int pet_read_db_sub(struct config_setting_t *it, int n, const char *source) {
	...
	if (!libconfig->setting_lookup_int(it, "Id", &i32)) {
		ShowWarning("pet_read_db_sub: Sem ID em \"%s\", entrada #%d, ignorado.\n", source, n);
		return 0;
	}
	pet->db[n].class_ = i32;

	if (!libconfig->setting_lookup_string(it, "SpriteName", &str) || !*str ) {
		ShowWarning("pet_read_db_sub: Faltando SpriteName no pet %d em \"%s\", ignorado.\n", pet->db[n].class_, source);
		return 0;
	}
	safestrncpy(pet->db[n].name, str, sizeof(pet->db[n].name));

	if (!libconfig->setting_lookup_string(it, "Name", &str) || !*str) {
		ShowWarning("pet_read_db_sub: Faltando Name no pet %d em \"%s\", ignorado.\n", pet->db[n].class_, source);
		return 0;
	}
	...
	if (libconfig->setting_lookup_string(it, "TamingItem", &str)) {
		if (!(data = itemdb->name2id(str))) {
			ShowWarning("pet_read_db_sub: Item invalido '%s' no pet %d em \"%s\", definido para 0.\n", str, pet->db[n].class_, source);
		} else {
...
/**
 * Read Pet Evolution Database [Dastgir]
...
void pet_read_db_sub_evolution(struct config_setting_t *t, int n) {
	...
	while ((pett = libconfig->setting_get_elem(t, i))) {
		if (config_setting_is_group(pett)) {
			...
			if (!(data = itemdb->name2id(str))) {
				ShowWarning("pet_read_evolve_db_sub: Ovo '%s' invalido no Pet #%d, ignorado.\n", str, pet->db[n].class_);
				return;
			} else {
			...
			while ((item = libconfig->setting_get_elem(pett, j))) {
				...
				if (!data) {
					ShowWarning("pet_read_evolve_db_sub: Item %s requerido nao encontrado para ovo %d\n", str, ped.petEggId);
					j++;
					continue;
				}
				list.id = data->nameid;
				...
				if (quantity <= 0) {
					ShowWarning("pet_read_evolve_db_sub: Quantidade %d invalida para ovo %d\n", quantity, ped.petEggId);
					j++;
					continue;
				}
...
void pet_read_db_clear(void) {
	...
	// Remover dados anteriores no caso reloaddb ser ativado
	for (i = 0; i < MAX_PET_DB; i++) {
		int j;
