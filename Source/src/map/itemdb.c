...
/**
 * Searches for the itemdb_option data.
...
void itemdb_read_groups(void) {
	...
	const char *config_filename = "Database/Item_DB/GroupList.conf"; // FIXME hardcoded name
	const char *itname;
	int i = 0, count = 0, c;
...
void itemdb_read_packages(void) {
	...
	const char *config_filename = "Database/Item_DB/Packages.conf"; // FIXME hardcoded name
	const char *itname;
...
void itemdb_read_options(void)
{
	...
	int index = 0, count = 0;
	const char *filepath = "Database/Item_DB/EffectOptions.conf";
...
void itemdb_read_chains(void) {
	...
	const char *config_filename = "Database/Item_DB/ChainList.conf"; // FIXME hardcoded name
	int i = 0, count = 0;
...
void itemdb_read(bool minimal) {
	int i;
	struct DBData prev;
	const char *filename = "Item_DB/ItemList.conf";
...
bool itemdb_items_identical(const struct item *a, const struct item *b, bool stack_match) {
	...
	if (a->nameid != b->nameid || a->identify != b->identify || a->refine != b->refine || a->attribute != b->attribute) {
		return false;
	}
	if (stack_match) {
		if (a->expire_time != b->expire_time || a->bound != b->bound || a->unique_id != b->unique_id) {
			return false;
		}
	}
...
void itemdb_fill_produceinfo(struct item *item, int char_id) {
	STATIC_ASSERT(MAX_SLOTS >= 4, "MAX_SLOTS deve ter 4");
	nullpo_retv(item);
