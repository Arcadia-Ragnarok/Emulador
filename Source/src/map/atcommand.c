...
#define CHECK_EOS(p) do { \
	if(*(p) == 0){ \
		clif->message(fd, "Inesperado final de string");\
		return false;\
	} \
...
ACMD(cvcoff) {
	...
	clif->maptypeproperty2(&sd->bl, ALL_SAMEMAP);
	map->foreachinmap(atcommand->stopattack, sd->bl.m, BL_CHAR, 0);
	clif->message(fd, msg_txt(137)); // CvC: Off.
...
ACMD(cvcon) {
	...
	clif->map_property_mapall(sd->bl.m, MAPPROPERTY_AGITZONE);
	clif->maptypeproperty2(&sd->bl, ALL_SAMEMAP);
	clif->message(fd, msg_txt(138)); // CvC: On.
...
ACMD(bodystyle)
{
	...
	if (*message == '\0' || sscanf(message, "%d", &body_style) < 1) {
		sprintf(atcmd_output, "Entre com o body style (uso: @bodystyle <body ID: %d-%d>).", MIN_BODY_STYLE, MAX_BODY_STYLE);
		clif->message(fd, atcmd_output);
		return false;
	}
...
/*==========================================
 * @reloadatcommand - reloads Config/atcommand.cs Config/groups.conf
 *------------------------------------------*/
ACMD(reloadatcommand) {
	struct config_t run_test;

	if (!libconfig->load_file(&run_test, "Config/System/Groups.cs")) {
...
/*==========================================
 * @reloadstatusdb - reloads job_db1.txt LvStatus.txt job_db2-2.txt refine_db.txt SizeFix.txt
 *------------------------------------------*/
ACMD(reloadstatusdb) {
	status->readdb();
...
/*==========================================
 * @reloadpcdb - reloads exp.txt skill_tree.txt AttrFix.txt statpoint.txt
 *------------------------------------------*/
ACMD(reloadpcdb)
{
	pc->readdb();
...
ACMD(mapinfo)
{
	...
	if (map->list[m_id].flag.noautoloot)
		strcat(atcmd_output, msg_txt(1063)); // NoAutoloot |
	if (map->list[m_id].flag.noviewid != EQP_NONE)
		strcat(atcmd_output, msg_txt(1079)); // NoViewID |
	clif->message(fd, atcmd_output);
...
ACMD(servertime) {
	...
	if (pc->day_timer_tid != INVALID_TIMER && pc->night_timer_tid != INVALID_TIMER) {
		...
		sprintf(temp, msg_txt(238), txt_time(fd,timer_data2->interval / 1000)); // Game time: A day cycle has a normal duration of %s.
		clif->message(fd, temp);
	} else {
		if (map->night_flag == 0)
...
ACMD(exp)
{
	...
	sprintf(atcmd_output, msg_txt(1148), sd->status.base_level, percentb, sd->status.job_level, percentj); // Base Level: %d (%.3f%%) | Job Level: %d (%.3f%%)
	clif->message(fd, atcmd_output);
	return true;
}
...
ACMD(storeall)
{
	...
	if (sd->storage.received == false) {
		clif->message(fd, msg_txt(27)); // "Storage has not been loaded yet"
		return false;
	}
...
ACMD(skillid)
{
	...
	for (data = iter->first(iter,&key); iter->exists(iter); data = iter->next(iter,&key)) {
		...
		if (strnicmp(key.str, message, skillen) == 0 || strnicmp(skill_desc, message, skillen) == 0) {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1164), skill_id, skill_desc, key.str); // skill %d: %s (%s)
			clif->message(fd, atcmd_output);
		} else if (found < MAX_SKILLID_PARTIAL_RESULTS && (stristr(key.str, message) != NULL || stristr(skill_desc, message) != NULL)) {
			snprintf(partials[found], MAX_SKILLID_PARTIAL_RESULTS_LEN, msg_txt(1164), skill_id, skill_desc, key.str);
			found++;
		}
	}
...
ACMD(skilltree)
{
	...
	for (j = 0; j < VECTOR_LENGTH(entry->need); j++) {
		...
		if (pc->checkskill(sd, req->id) < req->lv) {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1170), req->lv, skill->get_desc(req->id)); // Player requires level %d of skill %s.
			clif->message(fd, atcmd_output);
			meets = 0;
		}
	}
...
ACMD(mobinfo)
{
	...
	for (k = 0; k < count; k++) {
		...
		// stats
		if (monster->mexp)
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1240), monster->name, monster->jname, monster->sprite, monster->vd.class); // MVP Monster: '%s'/'%s'/'%s' (%d)
		else
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1241), monster->name, monster->jname, monster->sprite, monster->vd.class); // Monster: '%s'/'%s'/'%s' (%d)
		clif->message(fd, atcmd_output);

		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1242), monster->lv, monster->status.max_hp, base_exp, job_exp, MOB_HIT(monster), MOB_FLEE(monster)); //  Lv:%d  HP:%d  Base EXP:%u  Job EXP:%u  HIT:%d  FLEE:%d
		clif->message(fd, atcmd_output);

		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1243), //  DEF:%d  MDEF:%d  STR:%d  AGI:%d  VIT:%d  INT:%d  DEX:%d  LUK:%d
		...
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1291), //  ATK : %d~%d MATK : %d~%d Range : %d~%d~%d  Size : %s  Race : %s  Element : %s(Lv : %d)
		...
		// mvp
		if (monster->mexp) {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1247), monster->mexp); //  MVP Bonus EXP:%u
			clif->message(fd, atcmd_output);
...
ACMD(homlevel) {
	...
	if ((htype = homun->class2type(hd->homunculus.class_)) == HT_INVALID) {
		ShowError("atcommand_homlevel: Classe homun invalida %d (player %s)\n", hd->homunculus.class_,sd->status.name);
		return false;
	}
	...
	switch( htype ) {
		...
		case HT_S:
			...
			break;
		default:
			ShowError("atcommand_homlevel: desconhecido htype '%d'\n",htype);
			return false;
	}
...
ACMD(version) {
	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1296), sysinfo->is64bit() ? 64 : 32, sysinfo->platform()); // %d-bit for %s
	clif->message(fd, atcmd_output);
...
#define SETFLAG( cmd ) do { \
	if (strcmp( flag_name , #cmd ) == 0) { \
		...
		safesnprintf(atcmd_output, sizeof(atcmd_output),"[ @mapflag ] %s flag definido para %s valor = %hd",#cmd,flag?"On":"Off",flag); \
		clif->message(sd->fd,atcmd_output); \
		return true; \
	} \
} while(0)
		...
		for (i = 0; i < atcommand->binding_count; i++) {
			if (gm_lvl >= ((type == COMMAND_ATCOMMAND) ? atcommand->binding[i]->group_lv : atcommand->binding[i]->group_lv_char)
				...
				if (count_bind == 0) {
					...
					line_buff[CHATBOX_SIZE - 1] = 0;
					clif->message(fd, "------------------");
					clif->message(fd, "Comandos personalizados:");
				}
				if (slen + cur - line_buff >= CHATBOX_SIZE) {
					clif->message(fd, line_buff);
					cur = line_buff;
...
ACMD(set)
{
	...
	switch( data->type ) {
		case C_INT:
			...
			break;
		case C_STR:
			safesnprintf(atcmd_output, sizeof(atcmd_output),msg_txt(1374),reg,data->u.mutstr); // %s value is now :%s
			break;
		case C_CONSTSTR:
...
ACMD(costume)
{
	...
	if (!*message) {
		...
		clif->message(sd->fd, msg_txt(1472)); // - Available Costumes
...
ACMD(cddebug) {
	...
	if (!(cd = idb_get(skill->cd_db,sd->status.char_id))) {
		clif->message(fd,"Nenhuma lista cooldown encontrada");
	} else {
		clif->messages(fd,"Encontrado %d registros cooldowns",cd->cursor);
		for(i = 0; i < cd->cursor; i++) {
			if( cd->entry[i] ) {
				const struct TimerData *td = timer->get(cd->entry[i]->timer);

				if( !td || td->func != skill->blockpc_end ) {
					clif->messages(fd,"Encontrada entradas invalidas no slot %d para habilidade %s",i,skill->dbs->db[cd->entry[i]->skidx].name);
	...
	if (!cd || (*message && !strcmpi(message,"reset"))) {
		for (i = 0; i < MAX_SKILL_DB; i++) {
			if( sd->blockskill[i] ) {
				clif->messages(fd,"Encontrada habilidade '%s', desbloqueando...",skill->dbs->db[i].name);
...
void atcommand_basecommands(void) {
	...
	for( i = 0; i < ARRAYLENGTH(atcommand_base); i++ ) {
		if(!atcommand->add(atcommand_base[i].command,atcommand_base[i].func,false)) { // Should not happen if atcommand_base[] array is OK
			ShowDebug("atcommand_basecommands: duplicado ACMD_DEF para '%s'.\n", atcommand_base[i].command);
			continue;
		}
	}
...
bool atcommand_exec(const int fd, struct map_session_data *sd, const char *message, bool player_invoked) {
	...
	if (player_invoked && atcommand->binding_count > 0) {
		...
		if (binding != NULL && binding->npc_event[0] != '\0' && (
			...
			/* log only if this command should be logged */
			if (binding->log) {
		...
		for (i = 0; i < map->list[sd->bl.m].zone->disabled_commands_count; i++) {
			if (info->func == map->list[sd->bl.m].zone->disabled_commands[i]->cmd) {
				if (pc_get_group_level(sd) < map->list[sd->bl.m].zone->disabled_commands[i]->group_lv) {
					clif->messagecolor_self(sd->fd, COLOR_RED, "Este comando esta desabilitado nesse mapa");
					return true;
				}
	...
	if ((info->func(fd, ssd, command, params,info) != true)) {
		...
		if (info->func == atcommand_autotrade) {
			return true;
		}
	#endif
...
void atcommand_db_load_groups(GroupSettings **groups, struct config_setting_t **commands_, size_t sz)
{
	...
	for (atcmd = dbi_first(iter); dbi_exists(iter); atcmd = dbi_next(iter)) {
		...
		for (i = 0; i < sz; i++) {
			...
			if (group == NULL) {
				ShowError("atcommand_db_load_groups: group NULL\n");
				continue;
			}
