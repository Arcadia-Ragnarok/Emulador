...
void chrif_recvmap(int fd) {
	...
	if (battle_config.etc_log)
		ShowStatus("Recebendo mapas de %u.%u.%u.%u:%u (%d mapas)\n", CONVIP(ip), port, j);
...
void chrif_removemap(int fd) {
	...
	if(battle_config.etc_log)
		ShowStatus("mapas removidos do servidor %u.%u.%u.%u:%u (%d mapas)\n", CONVIP(ip), port, j);
}
...
void chrif_recvfamelist(int fd) {
	...
	ShowInfo("Recebida lista da fama de '"CL_WHITE"%d"CL_RESET"' pesonagens.\n", total);
}
...
bool chrif_load_scdata(int fd) {
	...
	for (i = 0; i < count; i++) {
		...
		status->change_start(NULL, &sd->bl, (sc_type)data->type, 10000, data->val1, data->val2, data->val3, data->val4, data->tick, SCFLAG_NOAVOID|SCFLAG_FIXEDTICK|SCFLAG_LOADED|SCFLAG_FIXEDRATE);
	}
...
int check_connect_char_server(int tid, int64 tick, int id, intptr_t data) {
	...
	if ( chrif->fd <= 0 || sockt->session[chrif->fd] == NULL ) {
		if ( !displayed ) {
			ShowStatus("Tentando conectar ao char-server.\n");
			displayed = 1;
		}
...
{
	if( !chrif->isconnected() ) {
		ShowError("MAYDAY! Falha ao deletar o status %d do CID:%d/AID:%d\n",type,char_id,account_id);
		return;
	}
