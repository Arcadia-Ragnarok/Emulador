...
int npc_timerevent_export(struct npc_data *nd, int i)
{
	...
	lname = nd->u.scr.label_list[i].name;
	pos = nd->u.scr.label_list[i].pos;
	if (sscanf(lname, "OnTimer%d%n", &t, &len) == 1 && len < NPC_NAME_LENGTH && lname[len] == '\0') {
		// Timer event
...
void npc_parsename(struct npc_data* nd, const char* name, const char* start, const char* buffer, const char* filepath) {
	...
	if( p ) { // <Display name>::<Unique name>
		...
		if( len > NPC_NAME_LENGTH ) {
			ShowWarning("npc_parsename: Nome de exibicao '%s' e muito longo (len=%u) no arquivo '%s', linha '%d'. Compactando para %d caracteres.\n", name, (unsigned int)len, filepath, strline(buffer,start-buffer), NAME_LENGTH);
		...
		len = strlen(p+2);
		if( len > NPC_NAME_LENGTH )
			ShowWarning("npc_parsename: Nome unico '%s' e muito longo (len=%u) no arquivo '%s', linha '%d'. Compactando para %d caracteres.\n", name, (unsigned int)len, filepath, strline(buffer,start-buffer), NAME_LENGTH);
		safestrncpy(nd->exname, p+2, sizeof(nd->exname));
	} else {// <Display name>
		size_t len = strlen(name);
		if( len > NPC_NAME_LENGTH )
			ShowWarning("npc_parsename: Nome '%s' e muito longo (len=%u) no arquivo '%s', linha '%d'. Compactando para %d caracteres.\n", name, (unsigned int)len, filepath, strline(buffer,start-buffer), NAME_LENGTH);
		safestrncpy(nd->name, name, sizeof(nd->name));
...
int npc_parseview(const char* w4, const char* start, const char* buffer, const char* filepath) {
	...
	if(!npc->viewisid(viewid))
	{
		...
		val = atoi(w4);
		ShowWarning("npc_parseview: Usando ID de NPC '%d' no arquivo '%s', linha '%d' ISSO NAO E RECOMENDADO.\n", val, filepath, strline(buffer, start-buffer));
	}
...
const char *npc_parse_shop(const char *w1, const char *w2, const char *w3, const char *w4, const char *start, const char *buffer, const char *filepath, int *retval)
{
	...
	if( strcmp(w1,"-") == 0 ) {
		...
		if (dir < 0 || dir > 7) {
			ShowError("npc_parse_shop: Direcao de posicao do NPC invalida '%d' no arquivo '%s', linha '%d'.\n", dir, filepath, strline(buffer, start-buffer));
			if (retval) *retval = EXIT_FAILURE;
			return strchr(start,'\n');//continue
		}
	...
	if( m != -1 && ( x < 0 || x >= map->list[m].xs || y < 0 || y >= map->list[m].ys ) ) {
		ShowError("npc_parse_shop: out-of-bounds coordenadas (\"%s\",%d,%d), mapa %dx%d, arquivo '%s', linha '%d'\n", map->list[m].name, x, y, map->list[m].xs, map->list[m].ys,filepath,strline(buffer,start-buffer));
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start,'\n');//try next
	}
...
const char *npc_parse_mob(const char *w1, const char *w2, const char *w3, const char *w4, const char *start, const char *buffer, const char *filepath, int *retval)
{
	...
	char mapname[32], mobname[NPC_NAME_LENGTH];
	...
	if( mob->db_checkid(class_) == 0 ) {
		ShowError("npc_parse_mob: ID de monstro desconhecido %d no arquivo '%s', linha '%d'.\n", class_, filepath, strline(buffer,start-buffer));
		if (retval) *retval = EXIT_FAILURE;
		return strchr(start,'\n');// skip and continue
	}
	...
	if (!strcmpi(w3, "nosave")) {
		...
		if (state == 0)
			...
			if (!map->list[m].save.map) {
				ShowWarning("npc_parse_mapflag: Ponto de retorno especificado no mapa '%s' para o mapflag 'nosave' nao encontrado no arquivo '%s', linha '%d'.\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n", savemap, filepath, strline(buffer,start-buffer), w1, w2, w3, w4);
				if (retval) *retval = EXIT_FAILURE;
				map->list[m].save.x = -1;
				map->list[m].save.y = -1;
			}
...
int npc_parsesrcfile(const char* filepath, bool runOnInit) {
	...
	buffer[len] = '\0';
	if( ferror(fp) ) {
		ShowError("npc_parsesrcfile: Falha na leitura do arquivo '%s' - %s\n", filepath, strerror(errno));
		aFree(buffer);
		fclose(fp);
		return EXIT_FAILURE;
	}
	...
	for( p = script->skip_space(buffer); p && *p ; p = script->skip_space(p) ) {
		...
		{// w1 = <map name>,<x>,<y>,<facing>
			...
			if (x < 0 || x >= map->list[m].xs || y < 0 || y >= map->list[m].ys) {
				ShowError("npc_parsesrcfile: Coordenadas desconhecidas ('%d', '%d') para mapa '%s' no arquivo '%s', linha '%d'. Pulando linha...\n", x, y, mapname, filepath, strline(buffer,p-buffer));
				success = EXIT_FAILURE;
				if( strcmp(w2,"script") == 0 && count > 3 ) {
					if((p = npc->skip_script(p,buffer,filepath, &success)) == NULL) {
						break;
...
int npc_script_event(struct map_session_data* sd, enum npce_event type)
{
	...
	if (type == NPCE_MAX)
		return 0;
	if (!sd) {
		ShowError("npc_script_event: NULL sd. Tipo de evento %u\n", type);
		return 0;
	}
