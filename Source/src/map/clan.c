...
bool clan_join(struct map_session_data *sd, int clan_id) {
	...
	if (sd->status.guild_id > 0 || sd->guild != NULL) {
		ShowError("clan_join: O jogador ja se juntou ao clan. char_id: %d\n", sd->status.char_id);
		return false;
	...
	if (c == NULL) {
		ShowError("clan_join: ID de clan invalido: %d\n", clan_id);
		return false;
	}
	...
	if (clan->getindex(c, sd->status.char_id) != INDEX_NOT_FOUND) {
		ShowError("clan_join: O jogador ja se juntou a este clan. char_id: %d clan_id: %d\n", sd->status.char_id, clan_id);
		return false;
	}

	if (VECTOR_LENGTH(c->members) >= c->max_member || c->member_count >= c->max_member) {
		ShowError("clan_join: Clan '%s' alcancou o maximo de capacidade!\n", c->name);
		return false;
	}
...
void clan_member_online(struct map_session_data *sd, bool first) {
	...
	if (sd->status.clan_id < 0) {
		ShowError("clan_member_online: Id de clan invalido, alterado para '0'. clan_id='%d' char_id='%d'\n", sd->status.clan_id, sd->status.char_id);
		sd->status.clan_id = 0;
		return;
	}
	...
	if (c->max_member <= c->member_count || VECTOR_LENGTH(c->members) >= c->max_member) {
		ShowError("Clan System: Mais membros do que o maximo permitido no clan #%d\n", c->clan_id);
		return;
	}
...
int clan_inactivity_kick(int tid, int64 tick, int id, intptr_t data) {
	...
	if ((c = clan->search(id)) != NULL) {
		if (!c->kick_time || c->tid != tid || tid == INVALID_TIMER || c->tid == INVALID_TIMER) {
			ShowError("Falha no temporizador (Time: %d) %d != %d", c->kick_time, c->tid, tid);
			Assert_report(0);
			return 0;
		}
...
int clan_request_kickoffline(int tid, int64 tick, int id, intptr_t data) {
	...
	if ((c = clan->search(id)) != NULL) {
		if (c->req_kick_tid != tid || c->req_kick_tid == INVALID_TIMER) {
			ShowError("Falha no temporizador %d != %d", c->tid, tid);
			return 0;
		}
	...
	if (!libconfig->setting_lookup_int(buff, "Icon", &b->icon)) {
		...
		if (libconfig->setting_lookup_string(buff, "Icon", &temp_str)) {
			if (*temp_str && !script->get_constant(temp_str, &b->icon)) {
				ShowWarning("Clan %d: Valor do icone Buff invalido. Definindo para SI_BLANK.\n", c->clan_id);
				b->icon = -1; // SI_BLANK
			}
		}
	}
...
int clan_read_db_sub(struct config_setting_t *settings, const char *source, bool reload) {
	...
	for (s = 0; s < total; s++) {
		...
		if (libconfig->setting_lookup_int(cl, "Id", &id)) {
			if (id <= 0) {
				ShowError("clan_read_db: Id %d de Clan Invalido, ignorando...\n", id);
				return false;
			}

			if (clan->search(id) != NULL) {
				ShowError("clan_read_db: Id para o Clan %d duplicado, ignorando...\n", id);
				return false;
			}
		} else {
			ShowError("clan_read_db: nao encontrado 'Id' para clan #%d\n", s);
			return false;
		}

		if (libconfig->setting_lookup_string(cl, "Const", &aConst)) {
			if (!*aConst) {
				ShowError("clan_read_db: Const '%s' de Clan Invalido, ignorando...\n", aConst);
				return false;
			}

			if (strlen(aConst) > NAME_LENGTH) {
				ShowError("clan_read_db: O nome do Clan '%s' mais longo do que %d caracteres, ignorando...\n", aConst, NAME_LENGTH);
				return false;
			}

			if (clan->searchname(aConst) != NULL) {
				ShowError("clan_read_db: Duplicado Const para o Clan '%s', skipping...\n", aConst);
				return false;
			}
		} else {
			ShowError("clan_read_db: nao encontrado 'Const' para clan #%d\n", s);
			return false;
		}

		if (libconfig->setting_lookup_string(cl, "Name", &aName)) {
			if (!*aName) {
				ShowError("clan_read_db: Nome '%s' de Clan Invalido, ignorando...\n", aName);
				return false;
			}
			...
			if (clan->searchname(aName) != NULL) {
				ShowError("clan_read_db: Nome para o Clan '%s' duplicado, ignorando...\n", aName);
				return false;
			}
		} else {
			ShowError("clan_read_db: nao encontrado 'Name' para clan #%d\n", s);
			return false;
		}

		if (!libconfig->setting_lookup_string(cl, "Leader", &aLeader)) {
			ShowError("clan_read_db: nao encontrado 'Leader' para clan #%d\n", s);
			return false;
		}

		if (!libconfig->setting_lookup_string(cl, "Map", &aMap)) {
			ShowError("clan_read_db: nao encontrado 'Map' para clan #%d\n", s);
			return false;
		}
		...
		if (libconfig->setting_lookup_int(cl, "MaxMembers", &max_members)) {
			if (max_members > 0) {
				...
				ShowError("clan_read_db: Clan #%d tem valor invalido para a configuracao 'MaxMembers', dedinido para 'clan->max'...\n", id);
		...
		if (allies != NULL) {
			...
			if (a > 0 && clan->max_relations > 0) {
				...
				if (a > clan->max_relations) {
					ShowWarning("clan_read_db: Clan %d tem mais aliados(%d) do que permitido(%d), lendo apenas o primeiro %d...\n", c->clan_id, a, clan->max_relations, clan->max_relations);
				...
				for (i = 0; i < a; i++) {
					...
					if ((allyConst = libconfig->setting_get_string_elem(allies, i)) != NULL) {
						...
						if (j != VECTOR_LENGTH(c->allies)) {
							ShowError("clan_read_db: Entrada duplicada '%s' nos enimigos do Clan %d em '%s', ignorando...\n", allyConst, c->clan_id, source);
							continue;
						} else if (strcmp(allyConst, c->constant) == 0) {
							ShowError("clan_read_db: Os Clans nao podem ser aliados de si mesmos! ID do Clan: %d, em '%s'\n", c->clan_id, source);
							continue;
						}
		...
		if (antagonists != NULL) {
			...
			if (a > 0 && clan->max_relations > 0) {
				...
				if (a > clan->max_relations) {
					ShowWarning("clan_read_db: Clan %d tem mais enimigos(%d) do que permitido(%d), lendo apenas o primeiro %d...\n", c->clan_id, a, clan->max_relations, clan->max_relations);
					a = clan->max_relations;
				}
				...
				for (i = 0; i < a; i++) {
					...
					if ((antagonistConst = libconfig->setting_get_string_elem(antagonists, i)) != NULL) {
						...
						if (j != VECTOR_LENGTH(c->antagonists)) {
							...
							ShowError("clan_read_db: Os clas nao podem ser inimigos de si mesmos! ID do Clan: %d, em '%s'\n", c->clan_id, source);
							continue;
						}
		...
		for (c_ok = dbi_first(iter); dbi_exists(iter); c_ok = dbi_next(iter)) {
			...
			while ( i > 0) {
				...
				if ((c = clan->searchname(r->constant)) == NULL) {
					ShowError("clan_read_db: Invalido aliados '%s' do clan %d em '%s', ignorando...\n", r->constant, c_ok->clan_id, source);
			...
			while ( i > 0) {
				...
				if ((c = clan->searchname(r->constant)) == NULL) {
					ShowError("clan_read_db: Invalido enimigos '%s' do clan %d em '%s', ignorando...", r->constant, c_ok->clan_id, source);
...
void clan_read_db(struct config_setting_t *settings, const char *source, bool reload) {
	...
	if ((clans = libconfig->setting_get_member(settings, "clans")) != NULL) {
		...
		ShowError("clan_read_db: Nao foi possivel localizar a configuracao 'clans' para '%s'.\n", source);
	}
}
...
bool clan_config_read(bool reload) {
	...
	const char *config_filename = "Config/System/Clans.cs"; // FIXME: hardcoded name
	...
	if (!libconfig->setting_lookup_int(settings, "InactivityCheckTime", &kickchecktime)) {
		ShowError("clan_config_read: nao encontrado InactivityCheckTime usando o valor oficial.\n");
		kickchecktime = 24;
	}
