...
/**
 * Delay na invocação no Túmulo [Guilherme Menaldo]
 */
...
int mvptomb_delayspawn(int tid, int64 tick, int id, intptr_t data) {
	...
	if (nd->u.tomb.spawn_timer != tid) {
		ShowError("mvptomb_delay_spawn: Tempo incorreto: %d != %d\n", tid, nd->u.tomb.spawn_timer);
		return 0;
	}
...
int mob_spawn_bg(const char* mapname, short x, short y, const char* mobname, int class_, const char* event, unsigned int bg_id)
{
	...
	if( (m = map->mapname2mapid(mapname)) < 0 ) {
		ShowWarning("mob_spawn_bg: Mapa [%s] nao econtrado.\n", mapname);
		return 0;
	}
...
void mob_read_db_mvpdrops_sub(struct mob_db *entry, struct config_setting_t *t)
{
	...
	while (idx < MAX_MVP_DROP && (drop = libconfig->setting_get_elem(t, i))) {
		...
		int value = 0;
		if (!id) {
			ShowWarning("mob_read_db: drop do MVP %s nao foi encontrado no monstro %d\n", name, entry->mob_id);
			i++;
			continue;
		}
		...
		if (value <= 0) {
			ShowWarning("mob_read_db: chance de drop errada %d para mvp drop %s no monstro %d\n", value, name, entry->mob_id);
			i++;
			continue;
		}
	...
	if (idx == MAX_MVP_DROP && libconfig->setting_get_elem(t, i)) {
		ShowWarning("mob_read_db: Muitas entradadas de mvp drops no monstro %d\n", entry->mob_id);
	}
}
...
void mob_read_db_drops_sub(struct mob_db *entry, struct config_setting_t *t)
{
	...
	while (idx < MAX_MOB_DROP && (drop = libconfig->setting_get_elem(t, i))) {
		...
		int value = 0;
		if (!id) {
			ShowWarning("mob_read_db: drop de item %s nao encontrado no monstro %d\n", name, entry->mob_id);
			i++;
			continue;
		}
		if (mob->get_const(drop, &i32) && i32 >= 0) {
		...
		if (value <= 0) {
			ShowWarning("mob_read_db: chance de drop errado %d para drop item %s no monstro %d\n", value, name, entry->mob_id);
			i++;
			continue;
		}
	...
	if (idx == MAX_MOB_DROP && libconfig->setting_get_elem(t, i)) {
		ShowWarning("mob_read_db: Muitos registros de drop no monstro %d\n", entry->mob_id);
	}
}
...
int mob_db_validate_entry(struct mob_db *entry, int n, const char *source)
{
	...
	if (entry->mob_id <= 1000 || entry->mob_id > MAX_MOB_DB) {
		ShowError("mob_db_validate_entry: ID de monstro invalido %d, deve estar em gama %d-%d.\n", entry->mob_id, 1000, MAX_MOB_DB);
		return 0;
	}
	if (pc->db_checkid(entry->mob_id)) {
		ShowError("mob_read_db_sub: ID de monstro invalido %d, reservado a classes de jogadores.\n", entry->mob_id);
		return 0;
	}
	if (entry->mob_id >= MOB_CLONE_START && entry->mob_id < MOB_CLONE_END) {
		ShowError("mob_read_db_sub: ID de monstro invalido %d. Alcance %d-%d resevado a clones de jogadores. Almente o MAX_MOB_DB (%d).\n", entry->mob_id, MOB_CLONE_START, MOB_CLONE_END-1, MAX_MOB_DB);
		return 0;
	}
...
int mob_read_db_sub(struct config_setting_t *mobt, int n, const char *source)
{
	...
	if (!libconfig->setting_lookup_int(mobt, "Id", &i32)) {
		ShowWarning("mob_read_db_sub: Perdido id em \"%s\", entrada #%d, ignorado.\n", source, n);
		return 0;
	}
	md.mob_id = i32;
	...
	if (!libconfig->setting_lookup_string(mobt, "SpriteName", &str) || !*str ) {
		if (!inherit) {
			ShowWarning("mob_read_db_sub: Perdido SpriteName no mob %d de \"%s\", ignorado.\n", md.mob_id, source);
			return 0;
		}
	} else {
	...
	if (!libconfig->setting_lookup_string(mobt, "Name", &str) || !*str ) {
		if (!inherit) {
			ShowWarning("mob_read_db_sub: Perdido Name no mob %d de \"%s\", ignorado.\n", md.mob_id, source);
			return 0;
		}
	} else {
	...
	if ((t = libconfig->setting_get_member(mobt, "Element")) && config_setting_is_list(t)) {
		...
		if (mob->get_const(libconfig->setting_get_elem(t, 0), &i32) && mob->get_const(libconfig->setting_get_elem(t, 1), &value)) {
			...
			ShowWarning("mob_read_db_sub: Perdido elemento do monstro %d.\n", md.mob_id);
...
int mob_read_libconfig(const char *filename, bool ignore_missing)
{
	...
	while ((t = libconfig->setting_get_elem(mdb, i++))) {
		...
		if (duplicate[mob_id]) {
			ShowWarning("mob_read_libconfig:%s: entrada duplicada com ID #%d (%s/%s)\n",
...
bool mob_readdb_mobavail(char* str[], int columns, int current)
{
	...
	if(mob->db(class_) == mob->dummy) {
		...
		ShowWarning("mob_readdb_mobavail: Desconhecido mob id %d.\n", class_);
		return false;
	}
...
int mob_read_randommonster(void)
{
	...
	for (i = 0; i < ARRAYLENGTH(mobfile) && i < MAX_RANDOMMONSTER; i++) {
		...
		fclose(fp);
		ShowStatus("Leitura de '"CL_WHITE"%u"CL_RESET"' registros em '"CL_WHITE"%s"CL_RESET"'.\n",count,mobfile[i]);
	}
	return 0;
}
...
bool mob_parse_row_chatdb(char** str, const char* source, int line, int* last_msg_id)
{
	...
	if (msg_id <= 0 || msg_id > MAX_MOB_CHAT)
	{
		if (msg_id != *last_msg_id) {
			ShowError("mob_chat: Invalido chat ID: %d em %s, linha %d\n", msg_id, source, line);
			*last_msg_id = msg_id;
		}
		return false;
	}
	...
	{
		if (msg_id != *last_msg_id) {
			ShowError("mob_chat: readdb: Messagem muito longa! Linha %d, id: %d\n", line, msg_id);
			*last_msg_id = msg_id;
		}
		return false;
	}
	...
	{
		ShowWarning("mob_parse_row_chatdb: Message vazia para id %d.\n", msg_id);
		return false;
	}
...
void mob_readchatdb(void) {
	...
	fp=fopen(filepath, "r");
	if(fp == NULL) {
		ShowWarning("mob_readchatdb: Arquivo nao encontrado \"%s\", ignorado.\n", filepath);
		return;
	}
	...
	while(fgets(line, sizeof(line), fp)) {
		...
		if( j < 2 || str[2]==NULL)
		{
			ShowError("mob_readchatdb: Numero de campos para habilidades insuficiente em %s, linha %u\n", arc, lines);
			continue;
		}
	...
	fclose(fp);
	ShowStatus("Leitura de '"CL_WHITE"%"PRIu32""CL_RESET"' registros em '"CL_WHITE"%s"CL_RESET"'.\n", count, arc);
}
...
void mob_readskilldb(void) {
	const char *filename[] = {"Mob_DB/MobSkill.conf"};
	int i;
