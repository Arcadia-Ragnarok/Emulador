...
void status_calc_bl_main(struct block_list *bl, /*enum scb_flag*/int flag)
{
	...
	**/
	...
	if(flag&SCB_LUK) {
		...
		flag|=SCB_BATK|SCB_CRI|SCB_FLEE2|SCB_MATK|SCB_HIT|SCB_FLEE;
	}
...
void status_calc_bl_(struct block_list *bl, enum scb_flag flag, enum e_status_calc_opt opt)
{
	...
	if( bl->type == BL_PC ) {
		...
		if(bst.rhw.atk2 != st->rhw.atk2 || bst.lhw.atk2 != st->lhw.atk2 || bst.rhw.atk != st->rhw.atk || bst.lhw.atk != st->lhw.atk)
...
void status_set_viewdata(struct block_list *bl, int class_)
{
	...
	case BL_NPC:
	{
		...
		if (vd != NULL) {
			...
			ShowError("status_set_viewdata (NPC): Sem visualizacao de dados para a classe %d (name=%s)\n", class_, nd->name);
		}
	}
		break;
		...
		case SC_DANCING:
			{
				...
				if (sce->val2) {
					...
					if (group == NULL) {
						ShowDebug("status_change_end: SC_DANCING esta faltando grupo de unidade de habilidade (val1=%d, val2=%d, val3=%d, val4=%d, timer=%d, tid=%d, char_id=%d, map=%s, x=%d, y=%d, prev=%s:%d, from=%s:%d). Please report this! (#3504)\n",
...
int status_change_timer(int tid, int64 tick, int id, intptr_t data)
{
	...
	bl = map->id2bl(id);
	if (!bl) {
		ShowDebug("status_change_timer: Id de ponteiro nulo %d data: %"PRIdPTR"\n", id, data);
		return 0;
	}
	sc = status->get_sc(bl);
	st = status->get_status_data(bl);
	...
	if (!sc || (sce = sc->data[type]) == NULL) {
		ShowDebug("status_change_timer: Id de ponteiro nulo %d data: %"PRIdPTR" bl-type: %u\n", id, data, bl->type);
		return 0;
	}
...
#define sc_timer_next(t,f,i,d) do { \
	if ((sce=sc->data[type])) \
		...
		ShowError("status_change_timer: Inesperado status NULL alterando %d data: %"PRIdPTR"\n", id, data); \
} while(0)
...
void status_read_job_db_sub(int idx, const char *name, struct config_setting_t *jdb)
{
	...
	if (libconfig->setting_lookup_string(jdb, "BaseExpGroup", &str) != 0) {
		...
		if (i == VECTOR_LENGTH(pc->class_exp_groups[CLASS_EXP_TABLE_BASE])) {
			ShowError("status_read_job_db: Grupo Desconhecido de Exp. Base '%s' fornecido para entrada '%s'\n", str, name);
		} else {
			pc->dbs->class_exp_table[idx][CLASS_EXP_TABLE_BASE] = &VECTOR_INDEX(pc->class_exp_groups[CLASS_EXP_TABLE_BASE], i);
		}
	} else {
		ShowError("status_read_job_db: Configuracao de BaseExpGroup nao encontrada para entrada '%s'\n", name);
	}
	...
	if (libconfig->setting_lookup_string(jdb, "JobExpGroup", &str) != 0) {
		...
		if (i == VECTOR_LENGTH(pc->class_exp_groups[CLASS_EXP_TABLE_JOB])) {
			ShowError("status_read_job_db: Grupo Desconhecido de Job Exp. Base '%s' fornecido para entrada '%s'\n", str, name);
		} else {
			pc->dbs->class_exp_table[idx][CLASS_EXP_TABLE_JOB] = &VECTOR_INDEX(pc->class_exp_groups[CLASS_EXP_TABLE_JOB], i);
		}
	} else {
		ShowError("status_read_job_db: Configuracao de JobExpGroup nao encontrada para entrada '%s'\n", name);
	}
	...
	if ((temp = libconfig->setting_get_member(jdb, "Inherit"))) {
		...
		while ((iname = libconfig->setting_get_string_elem(temp, nidx++))) {
			...
			if ((iclass = pc->check_job_name(iname)) == -1) {
				ShowWarning("status_read_job_db: '%s' tentando herdar o desconhecido '%s'!\n", name, iname);
				continue;
			}
	...
	if ((temp = libconfig->setting_get_member(jdb, "InheritHP"))) {
		...
		while ((iname = libconfig->setting_get_string_elem(temp, nidx++))) {
			int i, iidx, iclass, avg_increment, base;
			if ((iclass = pc->check_job_name(iname)) == -1) {
				ShowWarning("status_read_job_db: '%s' tentando herdar o desconhecido '%s' HP!\n", name, iname);
				continue;
			}
			iidx = pc->class2idx(iclass);
			for (i = 1; i <= 175 && status->dbs->HP_table[iidx][i]; i++) { //MAX_LEVEL
	...
	if ((temp = libconfig->setting_get_member(jdb, "InheritSP"))) {
		...
		while ((iname = libconfig->setting_get_string_elem(temp, nidx++))) {
			int i, iidx, iclass, avg_increment, base;
			if ((iclass = pc->check_job_name(iname)) == -1) {
				ShowWarning("status_read_job_db: '%s' tentando herdar o desconhecido '%s' SP!\n", name, iname);
				continue;
			}
			iidx = pc->class2idx(iclass);
			for (i = 1; i <= 175 && status->dbs->SP_table[iidx][i]; i++) { //MAX_LEVEL
	...
	if ((temp = libconfig->setting_get_member(jdb, "BaseASPD"))) {
		...
		while ((wpn = libconfig->setting_get_elem(temp, widx++))) {
			...
			if (w != wlen) {
				...
				ShowWarning("status_read_job_db: tipo de arma desconhecido '%s'!\n", wname);
			}
		}
	}
...
/*------------------------------------------
* DB reading.
* JobStatus.conf    - weight, hp, sp, aspd
* LvStatus.txt    - job level stat bonuses
* SizeFix.txt   - size adjustment table for weapons
...
void status_read_job_db(void) /* [malufett] */
{
	...
	while ((jdb = libconfig->setting_get_elem(job_db_conf.root, i++))) {
		...
		if ((class = pc->check_job_name(name)) == -1) {
			ShowWarning("pc_read_job_db: '%s' nome de classe desconhecida!\n", name);
			continue;
		}
...
bool status_readdb_job2(char* fields[], int columns, int current)
{
	...
	if (!pc->db_checkid(class)) {
		ShowWarning("status_readdb_job2: Invalido job class %d.\n", class);
		return false;
	}
	idx = pc->class2idx(class);
	...
	if ((rate=libconfig->setting_get_member(r, "Rates")) != NULL && config_setting_is_group(rate)) {
		...
		while ((t = libconfig->setting_get_elem(rate,i++)) != NULL && config_setting_is_group(t)) {
			...
			if (!strspn(&rlvl[strlen(rlvl) - 1], "0123456789") || (level = atoi(strncpy(lv, rlvl + 2, 3))) <= 0) {
				ShowError("status_readdb_refine_libconfig_sub: Formato de Nivel de Refinamento '%s' para entrada %s em \"%s\"... pulando.\n", rlvl, name, source);
				continue;
			}

			if (level <= 0 || level > MAX_REFINE) {
				ShowError("status_readdb_refine_libconfig_sub: Nivel de Refinamento '%s' para entrada %s em \"%s\"... pulando.\n", rlvl, name, source);
				continue;
			}
			...
			if (duplicate[level]) {
				ShowWarning("status_readdb_refine_libconfig_sub: duplicado rate '%s' para entrada %s em \"%s\", escrevendo...\n", rlvl, name, source);
			} else {
...
/**
 * Reads from a libconfig-formatted Refine.conf file.
 *
...
int status_readdb_refine_libconfig(const char *filename)
{
	...
	while((r = libconfig->setting_get_elem(refine_db_conf.root,i++))) {
		...
		if (type != 0) {
			if (duplicate[type-1]) {
				ShowWarning("status_readdb_refine_libconfig: entrada duplicada para %s em \"%s\", substituindo...\n", name, filename);
			} else {
