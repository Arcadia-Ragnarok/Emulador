...
/* returns name of current function being run, from within the stack [Ind] */
...
bool script_sprintf_helper(struct script_state *st, int start, struct StringBuf *out) {
	...
	while ((np = strchr(p, '%')) != NULL) {
		...
		if (*np == 'n') {
			ShowWarning("script_sprintf_helper: Formato %%n nao suportado! Ignorado...\n");
			script->reportsrc(st);
			lastarg = nextarg;
			p = np + 1;
			continue;
 ...
 * @param slot            as the slot of the item option (1 to MAX_ITEM_OPTIONS)
 * @param opt_index       as the index of the option available as "Id" in Database/item_options.conf.
...
BUILDIN(clan_master) {
	...
	if (nd == NULL) {
		...
		script_pushint(st, false);
		ShowError("buildin_clan_master: Recebido ID de clan invalido. %d\n", clan_id);
		return false;
	} else if (clan->search(clan_id) == NULL) {
		script_pushint(st, false);
		ShowError("buildin_clan_master: Recebido ID de Clan nao existente. %d\n", clan_id);
		return false;
	}
...
void script_parse_builtin(void) {
	...
	struct script_function BUILDIN[] = {
		...
		BUILDIN_DEF(countitem2,"viii" CARDSLOTS_CMDARGS),
		...
		BUILDIN_DEF(montransform, "vi?????"), // Monster Transform [malufett]
