...
#include <stdio.h>
#include <stdlib.h>
#include <sys/stat.h> // cache purposes
...
void findfile(const char *p, const char *pat, void (func)(const char*))
{
	...
	if (!dir) {
		ShowError("Nao foi possivel ler diretorio '%s'\n", path);
		return;
	}
...
unsigned int get_percentage(const unsigned int A, const unsigned int B)
{
	...
	if( B == 0 )
	{
		ShowError("get_percentage(): dividido por zero! (A=%u,B=%u)\n", A, B);
		return ~0U;
	}
	...
	if( result > UINT_MAX )
	{
		ShowError("get_percentage(): resultado da porcentagem muito alta! (A=%u,B=%u,result=%g)\n", A, B, result);
		return UINT_MAX;
	}
...
uint64 get_percentage64(const uint64 A, const uint64 B)
{
	...
	if( B == 0 )
	{
		ShowError("get_percentage(): dividido por zero! (A=%"PRIu64",B=%"PRIu64")\n", A, B);
		return ~0U;
	}
	...
	if( result > UINT_MAX )
	{
		ShowError("get_percentage(): resultado muito alto! (A=%"PRIu64",B=%"PRIu64",result=%g)\n", A, B, result);
		return UINT_MAX;
	}
