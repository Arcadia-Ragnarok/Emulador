...
int lclif_parse(int fd) {
	...
	if (sockt->session[fd]->flag.eof) {
		ShowInfo("Conexao fechada para '"CL_WHITE"%s"CL_RESET"'.\n", ip);
		sockt->close(fd);
		return 0;
	}
	...
	if ((sd = sockt->session[fd]->session_data) == NULL) {
		...
		if (login->config->ipban && !sockt->trusted_ip_check(ipl) && ipban->check(ipl)) {
			ShowStatus("Conexao recusada: IP nao altorizado (deny/allow, ip: %s).\n", ip);
	...
	for (i = 0; i < MAX_PROCESSED_PACKETS; ++i) {
		...
		case PACKET_STOPPARSE:
			return 0;
		case PACKET_UNKNOWN:
			ShowWarning("lclif_parse: Recebido packet nao suportado (packet 0x%04x, %d bytes recebidos), desconectado #%d.\n", (unsigned int)packet_id, packet_len, fd);
		...
		case PACKET_INVALIDLENGTH:
			ShowWarning("lclif_parse: Recebido packet 0x%04x leitura invalida (%d) 'packet_len', desconectado #%d.\n", (unsigned int)packet_id, packet_len, fd);
#ifdef DUMP_INVALID_PACKET
			ShowDump(RFIFOP(fd, 0), RFIFOREST(fd));
#endif
			sockt->eof(fd);
