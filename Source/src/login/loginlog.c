...
unsigned long loginlog_failedattempts(uint32 ip, unsigned int minutes) {
	...
	if (SQL_ERROR == SQL->Query(loginlog->sql_handle, "SELECT count(*) FROM `%s` WHERE `ip` = '%s' AND `rcode` = '1' AND `time` > NOW() - INTERVAL %u MINUTE", loginlog->dbs->log_login_db, sockt->ip2str(ip,NULL), minutes)) {
 ...
 */
bool loginlog_config_read_names(const char *filename, struct config_t *config) {
	...
	if ((setting = libconfig->lookup(config, "inter_configuration/database_names")) == NULL) {
		ShowError("loginlog_config_read: Configuracao 'database_names' nao encontrada!\n", filename);
		return false;
	}
 ...
 * @retval false in case of error.
 */
bool loginlog_config_read_log(const char *filename, struct config_t *config) {
	struct config_setting_t *setting = NULL;
	...
	if ((setting = libconfig->lookup(config, "inter_configuration/log/sql_connection")) == NULL) {
		ShowError("loginlog_config_read: Configuracao 'sql_connection' nao encontrada!\n", filename);
		return false;
	}

	libconfig->setting_lookup_mutable_string(setting, "log_db_hostname", loginlog->dbs->log_db_hostname, sizeof(loginlog->dbs->log_db_hostname));
	libconfig->setting_lookup_mutable_string(setting, "log_db_database", loginlog->dbs->log_db_database, sizeof(loginlog->dbs->log_db_database));
	libconfig->setting_lookup_mutable_string(setting, "log_db_username", loginlog->dbs->log_db_username, sizeof(loginlog->dbs->log_db_username));
	libconfig->setting_lookup_mutable_string(setting, "log_db_password", loginlog->dbs->log_db_password, sizeof(loginlog->dbs->log_db_password));
	libconfig->setting_lookup_mutable_string(setting, "log_login_db",    loginlog->dbs->log_login_db,    sizeof(loginlog->dbs->log_login_db));

	libconfig->setting_lookup_uint16(setting, "log_db_port", &loginlog->dbs->log_db_port);
