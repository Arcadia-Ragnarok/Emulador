...
bool loginlog_init(void) {
	...
	if (SQL_ERROR == SQL->Connect(sql_handle, log_db_username, log_db_password, log_db_hostname, log_db_port, log_db_database)) {
		Sql_ShowDebug(sql_handle);
		SQL->Free(sql_handle);
		exit(EXIT_FAILURE);
	}
...
bool loginlog_config_read_log(const char *filename, struct config_t *config) {
	...
	libconfig->setting_lookup_mutable_string(setting, "log_db_hostname", log_db_hostname, sizeof(log_db_hostname));
	libconfig->setting_lookup_mutable_string(setting, "log_db_database", log_db_database, sizeof(log_db_database));
...
bool loginlog_config_read(const char *filename) {
	...
	if (!loginlog_config_read_names(filename, &config)) {
		retval = false;
	}
	if (!loginlog_config_read_log(filename, &config)) {
		retval = false;
	}
