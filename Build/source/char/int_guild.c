	...
	for( g = DB->data2ptr(iter->first(iter, &key)); dbi_exists(iter); g = DB->data2ptr(iter->next(iter, &key)) )
	{
		...
		if (g->save_flag == GS_REMOVE) {
			...
			if (chr->show_save_log)
				ShowInfo("Guild Descarregada (%d - %s)\n", g->guild_id, g->name);
...
#ifdef NOISY
	ShowInfo("Salvamento de guild ("CL_BOLD"%d"CL_RESET" - flag 0x%x).\n",g->guild_id, flag);
#endif
...
#ifdef NOISY
	ShowInfo("Carregamento de Guild (%d)...\n", guild_id);
#endif
...
static int inter_guild_castle_tosql(struct guild_castle *gc)
{
	...
	if (SQL_ERROR == SQL->QueryStr(inter->sql_handle, StrBuf->Value(&buf)))
		...
		ShowInfo("Savo castelo de guild (%d)\n", gc->castle_id);
...
static struct guild_castle *inter_guild_castle_fromsql(int castle_id)
{
	...
	if (chr->show_save_log)
		ShowInfo("Carregado castelo (%d) da guilda (%d)\n", castle_id, gc->guild_id);
...
static int inter_guild_sql_init(void)
{
	...
	//Read exp file
	sv->readdb("Database", "Guild_DB/GuildExp.txt", ',', 1, 1, MAX_GUILDLEVEL, inter_guild->exp_parse_row);
...
static struct guild *inter_guild_create(const char *name, const struct guild_member *master)
{
	...
	int i=0;
#ifdef NOISY
	ShowInfo("Creado guild (%s)\n", name);
#endif
	nullpo_ret(name);
	nullpo_ret(master);
	if(inter_guild->search_guildname(name) != 0){
		ShowInfo("int_guild: nome de guild ja existe [%s]\n",name);
		return NULL;
	}
	...
	ShowInfo("Criada Guild %d - %s (Lider: %s)\n", g->guild_id, g->name, g->master);

	//Add to cache
	idb_put(inter_guild->guild_db, g->guild_id, g);

	if (inter->enable_logs)
		inter->log("guild %s (id=%d) criada pelo mestre %s (id=%d)\n",
...
static bool inter_guild_change_leader(int guild_id, const char *name, int len)
{
	...
	ShowInfo("int_guild: Lider alterado para %s (Guild %d - %s)\n",g->master, guild_id, g->name);
