...
{
	if (id < 0)
		ShowNotice("Enviando todos usuarios como offline.\n");
	else
		ShowNotice("Enviando todos usuarios de map-server %d como offline.\n",id);
...
static int char_mmo_char_tosql(int char_id, struct mmo_charstatus *p)
{
	...
	if (chr->show_save_log && save_status[0] != '\0')
		ShowInfo("Salvado personagem %d - %s:%s.\n", char_id, p->name, save_status);
	if (!errors)
...
static int char_memitemdata_to_sql(const struct item *p_items, int guid, enum inventory_table_type table)
{
	...
	ShowInfo("%s salvamento completo - guid: %d (substituido: %d, inserido: %d, apagado: %d)\n", tablename, guid, total_updates, total_inserts, total_deletes);
	...
	if (sex == 'M' || sex == 'F') {
		...
		if ((sex == 'M' && sd->sex == SEX_FEMALE)
		 || (sex == 'F' && sd->sex == SEX_MALE)) {
			...
			ShowInfo("Resetando sexo do personagem '%s' (CID: %d, AID: %d) 'U' imcompativel.\n", p->name, p->char_id, p->account_id);
		}
...
static int char_mmo_char_fromsql(int char_id, struct mmo_charstatus *p, bool load_everything)
{
	...
	if (chr->show_save_log)
		ShowInfo("Cerregamento do personagem (%d) requisitado\n", char_id);
...
static int char_make_new_char_sql(struct char_session_data *sd, const char *name_, int str, int agi, int vit, int int_, int dex, int luk, int slot, int hair_color, int hair_style, int starting_class, uint8 sex)
{
	...
	ShowInfo("Criado personagem: conta: %d, char: %d, slot: %d, name: %s, sex: %c\n", sd->account_id, char_id, slot, name, sex);
	return char_id;
}
...
static int char_delete_char_sql(int char_id)
{
	...
	if( ( char_del_level > 0 && base_level >= char_del_level )
	 || ( char_del_level < 0 && base_level <= -char_del_level )
			...
			ShowInfo("Delecao de personagem abortada: %s, BaseLevel: %i\n", name, base_level);
			return -1;
	}
...
#endif
	if (chr->show_save_log)
		ShowInfo("Carregado dados do personagem ("CL_BOLD"%d"CL_RESET")\n",sd->account_id);
...
static int char_parse_fromlogin_connection_state(int fd)
{
	switch (RFIFOB(fd,2)) {
	case 0:
		ShowStatus("Conectando ao login-server (#%d).\n", fd);
		loginif->on_ready();
		break;
	case 1: // Invalid username/password
		...
		ShowError("The communication passwords are set in Config/Servers/Map-Server.cs and /conf/char/char-server.conf\n");
		sockt->eof(fd);
		return 1;
	case 2: // IP not allowed
		ShowError("Can not connect to login-server.\n");
		ShowError("Please make sure your IP is allowed in Config/Connect/NetWork.conf\n");
		sockt->eof(fd);
		return 1;
	default:
...
static void char_parse_frommap_map_names(int fd, int id)
{
	...
	ShowStatus("Map-Server %d conectado: %d mapas, para o IP %u.%u.%u.%u port %d.\n",
			id, (int)VECTOR_LENGTH(chr->server[id].maps), CONVIP(chr->server[id].ip), chr->server[id].port);
	//ShowStatus("Map-server %d carregado.\n", id);
...
static void char_parse_frommap_update_ip(int fd, int id)
{
	...
	ShowInfo("Atualizando IP do map-server #%d para %u.%u.%u.%u.\n", id, CONVIP(chr->server[id].ip));
	RFIFOSKIP(fd,6);
}
...
static uint32 char_lan_subnet_check(uint32 ip)
{
	...
	if (sockt->lan_subnet_check(ip, &lan)) {
		ShowInfo("Checagem da Subnet [%u.%u.%u.%u]: Partidas "CL_CYAN"%u.%u.%u.%u/%u.%u.%u.%u"CL_RESET"\n", CONVIP(ip), CONVIP(lan.ip & lan.mask), CONVIP(lan.mask));
		return lan.ip;
	}
	ShowInfo("Checagem da Subnet [%u.%u.%u.%u]: "CL_CYAN"WAN"CL_RESET"\n", CONVIP(ip));
	return 0;
}
...
{// CH: <0829>.W <char id>.L <birth date:YYMMDD>.6B
	...
	ShowInfo(CL_RED"Deletar personagem: "CL_GREEN"%d (%d)"CL_RESET"\n", sd->account_id, char_id);
...
static void char_parse_char_select(int fd, struct char_session_data *sd, uint32 ipl)
{
	...
	ShowInfo("Selecionado personagem: (conta %d: %d - %s)\n", sd->account_id, slot, char_dat.name);
	...
	if (i < 0 || !cd->last_point.map) {
		...
		if (j == ARRAYLENGTH(chr->server)) {
			ShowInfo("Conexao fechada Closed. map serve não esta pronto.\n");
		...
		if (i < 0)
		{
			ShowInfo("Conexao Fechou. Nenhum servidor de mapa disponível, incapaz de achar map-server para '%s'.\n", mapindex_id2name(cd->last_point.map));
			chr->authfail_fd(fd, 1); // 1 = Server closed
			return;
		}
	}
...
#endif
	ShowInfo(CL_RED"Apagar personagem: "CL_GREEN"%d (%d)"CL_RESET"\n", sd->account_id, cid);
...
static int char_check_connect_login_server(int tid, int64 tick, int id, intptr_t data)
{
	...
	ShowInfo("Conectando ao login-server...\n");
 ...
 */
static bool char_sql_config_read(const char *filename)
{
	...
	ShowInfo("Carregamento completo: %s.\n", filename);
 ...
 * @retval false in case of error.
 */
static bool char_sql_config_read_registry(const char *filename, const struct config_t *config)
{
	const struct config_setting_t *setting = NULL;
 ...
 * @retval false in case of error.
 */
static bool char_sql_config_read_pc(const char *filename, const struct config_t *config)
{
	const struct config_setting_t *setting = NULL;
 ...
 * @retval false in case of error.
 */
static bool char_sql_config_read_guild(const char *filename, const struct config_t *config)
{
	const struct config_setting_t *setting = NULL;
 ...
 * @retval false in case of error.
 */
static bool char_config_read(const char *filename)
{
	struct config_t config;
 ...
 * @retval false in case of error.
 */
static bool char_config_read_top(const char *filename, const struct config_t *config)
{
	const struct config_setting_t *setting = NULL;
	...
	if (libconfig->setting_lookup_mutable_string(setting, "server_name", chr->server_name, sizeof(chr->server_name)) == CONFIG_TRUE) {
		ShowInfo("Nome do servidor %s\n", chr->server_name);
	} else {
		ShowWarning("char_config_read: server_name was not set! Defaulting to 'Arcadia'.\n");
...
/**
 * Reads the 'inter_connection' config entry and initializes required variables.
 *
 * @param filename Path to configuration file (used in error and warning messages).
 * @param config   The current config being parsed.
 *
 * @retval false in case of error.
 */
static bool char_config_read_inter(const char *filename, const struct config_t *config)
{
	const struct config_setting_t *setting = NULL;
	const char *str = NULL;
	...
	if ((setting = libconfig->lookup(config, "char_configuration/inter_connection")) == NULL) {
		ShowError("char_config_read: char_configuration/inter_connection was not found in %s!\n", filename);
		return false;
	}
 ...
 * @retval false in case of error.
 */
static bool char_config_read_database(const char *filename, const struct config_t *config)
{
	const struct config_setting_t *setting = NULL;
 ...
 * @retval false in case of error.
 */
static bool char_config_read_console(const char *filename, const struct config_t *config)
{
 ...
 * @retval false in case of error.
 */
static bool char_config_read_player(const char *filename, const struct config_t *config)
{
	bool retval = true;
 ...
 * @retval false in case of error.
 */
static bool char_config_read_player_fame(const char *filename, const struct config_t *config)
{
	const struct config_setting_t *setting = NULL;
 ...
 * @retval false in case of error.
 */
static bool char_config_read_player_deletion(const char *filename, const struct config_t *config)
{
	const struct config_setting_t *setting = NULL;
 ...
 * @retval false in case of error.
 */
static bool char_config_read_player_name(const char *filename, const struct config_t *config)
{
	const struct config_setting_t *setting = NULL;
 ...
 * @retval false in case of error.
 */
static bool char_config_read_player_new(const char *filename, const struct config_t *config)
{
 ...
 * @retval false in case of error.
 */
static bool char_config_read_permission(const char *filename, const struct config_t *config)
{
	const struct config_setting_t *setting = NULL;
...
int do_init(int argc, char **argv)
{
	...
	if ((sockt->naddr_ != 0) && (!login_ip || !chr->ip)) {
		...
		if (sockt->naddr_ > 1)
			ShowStatus("Multiplas interfaces detectada...  usando esse endereco de IP %s \n", ip_str);
		else
			ShowStatus("Definido %s endereco de IP padrao\n", ip_str);
		if (!login_ip) {
...
#endif
	ShowStatus("char-server "CL_GREEN"pronto"CL_RESET" (Acessando pela porta %d).\n\n", chr->port);
