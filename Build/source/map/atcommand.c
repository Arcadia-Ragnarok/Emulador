...
static const char *atcommand_msg(int msg_number)
{
	if (msg_number >= 0 && msg_number < MAX_MSG &&
	    atcommand->msg_table[msg_number] != NULL
	    && atcommand->msg_table[msg_number][0] != '\0')
...
#define GET_VALUE(p,num) do { \
	...
	if (type >= MIN_PACKET_DB && type <= MAX_PACKET_DB) {
		...
		if (len) {
			...
			len = packets->db[type];
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(904), type, len); // Packet 0x%x length: %d
			clif->message(fd, atcmd_output);
			return true;
		}
...
ACMD(cvcoff)
{
	if (!map->list[sd->bl.m].flag.cvc) {
		clif->message(fd, msg_txt( 141)); // CvC is already Off.
		return false;
	}
...
{
	if (map->list[sd->bl.m].flag.cvc) {
		clif->message(fd, msg_txt( 142)); // CvC is already On.
		return false;
	}
...
ACMD(setzone)
{
	...
	if (*message == '\0' || sscanf(message, fmt_str, zone_name) < 1) {
		clif->message(fd, msg_txt( 924)); // usage info
		return false;
	}
	...
	if (zone != NULL) {
		if (map->list[sd->bl.m].zone != zone) {
			...
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt( 925), zone_name);
		...
		clif->message(fd, msg_txt( 926)); // zone not found
		return false;
	}

	safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt( 927), prev_zone_name, zone_name);
...
ACMD(monster)
{
	...
	if (battle_config.etc_log)
		ShowInfo("%s monstro='%s' nome='%s' id=%d contagem=%d (%d,%d)\n", command, monster, name, mob_id, number, sd->bl.x, sd->bl.y);
...
/*==========================================
 * @reloadatcommand - reloads 
 Config/System/Atcommand.cs
 Config/System/Groups.cs
 *------------------------------------------*/
ACMD(reloadatcommand)
{
	struct config_t run_test;

	if (!libconfig->load_file(&run_test, "Config/System/Groups.conf")) {
		clif->message(fd, msg_txt(1036)); // Error reading Groups.cs, reload failed.
		return false;
	}
	...
	if (!libconfig->load_file(&run_test, map->ATCOMMAND_CONF_FILENAME)) {
		clif->message(fd, msg_txt(1037)); // Error reading Atcommand.cs, reload failed.
		return false;
	}
...
/*==========================================
 * @reloadstatusdb - reloads LvStatus.txt refine_db.txt size_fix.txt
 *------------------------------------------*/
ACMD(reloadstatusdb)
{
	status->readdb();
...
/*==========================================
 * @reloadpcdb - reloads JobExp.conf SkillList.conf AttrFix.txt StatPoint.txt
 *------------------------------------------*/
ACMD(reloadpcdb)
{
	pc->readdb();
...
ACMD(mapinfo)
{
	...
	if (map->list[m_id].flag.pairship_startable)
		strcat(atcmd_output, msg_txt( 1292)); // PrivateAirshipStartable |
	if (map->list[m_id].flag.pairship_endable)
		strcat(atcmd_output, msg_txt( 1293)); // PrivateAirshipEndable |
	clif->message(fd, atcmd_output);
...
ACMD(camerainfo)
{
	...
	if (sscanf(message, "%15f %15f %15f", &range, &rotation, &latitude) < 3) {
		clif->message(fd, msg_txt( 452));  // usage @camerainfo range rotation latitude
		return false;
	}
...
static void atcommand_config_read(const char *config_filename)
{
	...
	ShowStatus("Leitura de '"CL_WHITE"%d"CL_RESET"' pseudo comandos em '"CL_WHITE"%s"CL_RESET"'.\n", num_aliases, config_filename);
