...
static const char *skip_word(const char *p)
{
	...
	switch( *p ) {
		...
		case '#':// account variable
			p += ( p[1] == '#' ? 2 : 1 ); break;
		case '`':// instance variable [Spell Master]
			++p; break;
		case '.':// npc variable
			p += ( p[1] == '@' ? 2 : 1 ); break;
...
static const char *parse_simpleexpr_number(const char *p)
{
	...
	if (lli < INT_MIN) {
		lli = INT_MIN;
		script->disp_warning_message("parse_simpleexpr: parse_simpleexpr: underflow detectado, nivelando para o valor INT_MIN", p);
	} else if (lli > INT_MAX) {
		lli = INT_MAX;
		script->disp_warning_message("parse_simpleexpr: overflow parse_simpleexpr: underflow detectado, nivelando para o valor INT_MAX", p);
	}
...
static void read_constdb(bool reload)
{
	...
	safesnprintf(filepath, 256, "%s/Etc_DB/Constants.conf", map->db_path);
 ...
 */
static bool script_config_read(const char *filename)
{
...
static void do_final_script(void)
{
	...
	if (battle_config.etc_log)
	{
		...
		if(fp) {
			...
			ShowNotice("Manuscrito de str hash para hash_dump.txt\n");
 ...
 * !!Designed to be used with ItemOptions.conf only!!
 ...
 * @param slot            as the slot of the item option (1 to MAX_ITEM_OPTIONS)
 * @param opt_index       as the index of the option available as "Id" in Database/ItemOptions.conf.
 * @param value           as the value of the option type.
 *                        For IT_OPT_INDEX see "Name" in ItemOptions.conf
...
static BUILDIN(setbattleflag)
{
	...
	if (battle->config_set_value(flag, value) == 0)
		...
		ShowInfo("buildin_setbattleflag: battle_config flag '%s' fixado para '%s'.\n",flag,value);
