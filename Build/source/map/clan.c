...
static int clan_read_db_sub(struct config_setting_t *settings, const char *source, bool reload)
{
	...
	ShowStatus("Leitura de '" CL_WHITE "%d" CL_RESET "' registros em '" CL_WHITE "%d" CL_RESET "'.\n", valid, source);
	//ShowStatus("Leitura de '" CL_WHITE "%d" CL_RESET " 'validas de clans '" CL_WHITE "%d" CL_RESET "' de '" CL_WHITE "%s" CL_RESET "' entradas.\n", valid, total, source);
	return valid;
}
