...
#include "config/core.h" // AUTOLOOT_DISTANCE, DBPATH, DEFTYPE_MAX, DEFTYPE_MIN
#include "mob.h"
 ...
 * in Database/Item_DB/ChangeRate.txt
...
static void mob_readdb(void) {
	...
	mob->read_libconfig(filename, i > 0 ? true : false);
	mob->name_constants();
}
...
static int mob_read_randommonster(void)
{
	...
	for (i = 0; i < ARRAYLENGTH(mobfile) && i < MAX_RANDOMMONSTER; i++) {
		...
		fclose(fp);
		ShowStatus("Leitura de '"CL_WHITE"%u"CL_RESET"' registros em Database/'"CL_WHITE"%s"CL_RESET"'.\n",count,mobfile[i]);
	}
	return 0;
}
...
static void mob_readchatdb(void)
{
	...
	fclose(fp);
	ShowStatus("Leitura de '"CL_WHITE"%"PRIu32""CL_RESET"' registro em Database/'"CL_WHITE"%s"CL_RESET"'.\n", count, arc);
}
...
static void mob_readskilldb(void) {
	...
	if (battle_config.mob_skill_rate == 0) {
		ShowStatus("Habilidades de monstros estao desabilitada.\n");
		return;
	}
	if (exists(filename)) {
		i = 1;
	}
	mob->skill_db_libconfig(filename, i > 0 ? true : false);
}
