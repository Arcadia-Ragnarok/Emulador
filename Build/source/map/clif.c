...
static bool clif_setip(const char *ip)
{
	...
	ShowInfo("IP do Map Server: '"CL_WHITE"%s"CL_RESET"' -> '"CL_WHITE"%s"CL_RESET"'.\n", ip, sockt->ip2str(clif->map_ip, ip_str));
	return true;
}
...
static bool clif_setbindip(const char *ip)
{
	...
	if ( clif->bind_ip ) {
		char ip_str[16];
		ShowInfo("Bind IP do Map Server: '"CL_WHITE"%s"CL_RESET"' -> '"CL_WHITE"%s"CL_RESET"'.\n", ip, sockt->ip2str(clif->bind_ip, ip_str));
		return true;
	}
	ShowWarning("Failed to Resolve Map Server Address! (%s)\n", ip);
	return false;
...
static uint32 clif_refresh_ip(void)
{
	...
	if ( new_ip && new_ip != clif->map_ip ) {
		clif->map_ip = new_ip;
		ShowInfo("Resolucao de IP atualizando de [%s].\n", clif->map_ip_str);
		return clif->map_ip;
	}
	return 0;
}
...
static void clif_blname_ack(int fd, struct block_list *bl)
{
	...
	switch(bl->type) {
		...
		case BL_ELEM:
			...
			break;
		default:
			ShowError("clif_blname_ack: Mal formato %u(%d)\n", bl->type, bl->id);
			return;
	}
...
static bool clif_parse_roulette_db(void)
{
	...
	const char *config_filename = "Database/Item_DB/Roulette.conf";
		...
		if (sockt->session[fd]->flag.eof) {
			if (sd) {
				if (sd->state.autotrade) {
					...
					sd->fd = 0;
					ShowInfo("Personagem '"CL_WHITE"%s"CL_RESET"' saiu (usando @autotrade).\n", sd->status.name);
				} else
					if (sd->state.active) {
						...
						ShowInfo("Jogador AID:%d/CID:%d saiu.\n", sd->status.account_id, sd->status.char_id);
						map->quit(sd);
					}
			} else {
				ShowInfo("Finalizada conexao para '"CL_WHITE"%s"CL_RESET"'.\n", sockt->ip2str(sockt->session[fd]->client_addr, NULL));
			}
			sockt->close(fd);
			return 0;
		}
