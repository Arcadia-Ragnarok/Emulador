...
{
	ShowStatus("Evento '"CL_WHITE"OnInit"CL_RESET"' executado por '"CL_WHITE"%d"CL_RESET"' NPCs."CL_CLL"\n", npc->event_doall("OnInit"));
...
static int npc_selllist_sub(struct map_session_data *sd, struct itemlist *item_list, struct npc_data *nd)
{
	...
	char opt_index_str[NPC_NAME_LENGTH];
	char opt_value_str[NPC_NAME_LENGTH];
	int i, j;
	int key_nameid = 0;
	int key_amount = 0;
	int key_refine = 0;
...
static void npc_process_files(int npc_min)
{
	...
	ShowStatus("Carregamento de NPCs...\r");
	for( file = npc->src_files; file != NULL; file = file->next ) {
		ShowStatus("Carregamento arquivo NPC: %s"CL_CLL"\r", file->name);
	...
	ShowInfo("Carregando '"CL_WHITE"%d"CL_RESET"' NPCs:"CL_CLL"\n"
		...
		"\t-'"CL_WHITE"%d"CL_RESET"' Monstros em todas invocacoes\n",
		npc->npc_id - npc_min,
		npc->npc_warp,
		npc->npc_mob
	);
...
static int npc_reload(void)
{
	...
	npc->motd = npc->name2id("MOTD"); /* [Ind/Hercules] */
	...
	if( !intif->CheckForCharServer() ) {
		ShowStatus("Evento '"CL_WHITE"OnInterIfInit"CL_RESET"' executado por '"CL_WHITE"%d"CL_RESET"' NPCs.\n", npc->event_doall("OnInterIfInit"));
		ShowStatus("Evento '"CL_WHITE"OnInterIfInitOnce"CL_RESET"' executado por '"CL_WHITE"%d"CL_RESET"' NPCs.\n", npc->event_doall("OnInterIfInitOnce"));
	}
