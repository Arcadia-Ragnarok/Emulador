...
static int castle_guild_broken_sub(union DBKey key, struct DBData *data, va_list ap)
{
	...
	if (gc->guild_id == guild_id) {
		...
		// You can set all castle_events in the 'Database/Guild_DB/CastleList.txt'
...
static void guild_castle_map_init(void)
{
	...
	if (num > 0) {
		...
		if (intif->guild_castle_dataload(num, castle_ids))
			ShowStatus("Requisitado '"CL_WHITE"%d"CL_RESET"' castelos de guild para o char-server...\n", num);
		aFree(castle_ids);
	}
}
...
static int guild_castledataloadack(int len, const struct guild_castle *gc)
{
	...
	ShowStatus("Recebido '"CL_WHITE"%d"CL_RESET"' castelos de guild para o char-server.\n", n);
	return 0;
}
...
static void guild_agit_start(void)
{
	...
	ShowStatus("NPC_Event:[OnAgitStart] Executando (%d).\n",c);
}
...
static void guild_agit_end(void)
{
	...
	ShowStatus("NPC_Event:[OnAgitEnd] Executando (%d).\n",c);
}
...
static void guild_agit2_start(void)
{
	...
	ShowStatus("NPC_Event:[OnAgitStart2] Executando (%d).\n",c);
}
...
static void guild_agit2_end(void)
{
	...
	ShowStatus("NPC_Event:[OnAgitEnd2] Executando (%d).\n",c);
}
...
static void do_init_guild(bool minimal)
{
	...
	sv->readdb(map->db_path, "Guild_DB/CastleList.txt", ',', 4, 5, -1, guild->read_castledb);

	sv->readdb(map->db_path, "Guild_DB/GuildSkill.txt", ',', 2+MAX_GUILD_SKILL_REQUIRE*2, 2+MAX_GUILD_SKILL_REQUIRE*2, -1, guild->read_guildskill_tree_db); //guild skill tree [Komurka]
