...
#include "config/core.h" // CELL_NOSTACK, CIRCULAR_AREA, CONSOLE_INPUT, DBPATH
#include "map.h"
...
static void map_spawnmobs(int16 m)
{
	...
	if (battle_config.etc_log && k > 0) {
		ShowStatus("Mapa %s: com '"CL_WHITE"%d"CL_RESET"' mobs.\n",map->list[m].name, k);
	}
}
...
static int map_removemobs_timer(int tid, int64 tick, int id, intptr_t data)
{
	...
	if (battle_config.etc_log && count > 0)
		ShowStatus("Mapa %s: com '"CL_WHITE"%d"CL_RESET"' mobs removidos.\n",map->list[m].name, count);
...
static void map_iwall_remove(const char *wall_name)
{
	struct iwall_data *iwall;
	int16 i, x1, y1;
...
static void map_delmapid(int id)
{
	...
	ShowNotice("Removeido mapa [ %s ] da lista"CL_CLL"\n",map->list[id].name);
...
static int map_readallmaps(void)
{
	...
	snprintf(mapcachefilepath, 256, "%s/%s", map->db_path,"Map_DB/MapCache.dat");
		ShowStatus("Carregando mapas usando %s ...\n", mapcachefilepath);
	...
	for(i = 0; i < map->count; i++) {
		...
		// show progress
		if(map->enable_grf)
			ShowStatus("Carregado mapas [%i/%i]: %s"CL_CLL"\r", i, map->count, map->list[i].name);
	...
	ShowInfo("Carregado '"CL_WHITE"%d"CL_RESET"' mapas."CL_CLL"\n",map->count);
 ...
 */
static bool map_config_read_console(const char *filename, struct config_t *config)
{
 ...
 * @retval false in case of error.
 */
static bool map_config_read_connection(const char *filename, struct config_t *config)
{
	struct config_setting_t *setting = NULL;
...
/**
 * Reads 'inter_connection' and initializes required variables.
 *
 * @param filename Path to configuration file (used in error and warning messages).
 * @param config   The current config being parsed.
 *
 * @retval false in case of error.
 */
static bool map_config_read_inter(const char *filename, struct config_t *config)
{
	struct config_setting_t *setting = NULL;
	const char *str = NULL;
	char temp[24];
	...
	if ((setting = libconfig->lookup(config, "map_configuration/inter_connection")) == NULL) {
		ShowError("map_config_read: map_configuration/inter_connection was not found in %s!\n", filename);
		return false;
	}
 ...
 * @retval false in case of error.
 */
static bool map_config_read_database(const char *filename, struct config_t *config)
{
	struct config_setting_t *setting = NULL;
 ...
 * @retval false in case of error.
 */
static bool map_config_read_map_list(const char *filename, struct config_t *config)
{
	struct config_setting_t *setting = NULL;
	int i, count = 0;
 ...
 * @retval false in case of error.
 */
static bool map_config_read(const char *filename)
{
 ...
 * @retval false in case of error.
 */
static bool map_read_npclist(const char *filename)
{
	struct config_t config;
	struct config_setting_t *setting = NULL;
	//const char *import = NULL;
 ...
 * @retval false in case of error.
 */
static bool inter_config_read(const char *filename)
{
	struct config_t config;
 ...
 * @retval false in case of error.
 */
static bool inter_config_read_connection(const char *filename, const struct config_t *config)
{
	const struct config_setting_t *setting = NULL;
	...
	libconfig->setting_lookup_int(setting, "log_port", &logs->db_port);
	libconfig->setting_lookup_mutable_string(setting, "log_hostname", logs->db_ip, sizeof(logs->db_ip));
	libconfig->setting_lookup_mutable_string(setting, "log_username", logs->db_id, sizeof(logs->db_id));
	libconfig->setting_lookup_mutable_string(setting, "log_password", logs->db_pw, sizeof(logs->db_pw));
	libconfig->setting_lookup_mutable_string(setting, "log_database", logs->db_name, sizeof(logs->db_name));
 ...
 * @retval false in case of error.
 */
static bool inter_config_read_database_names(const char *filename, const struct config_t *config)
{
	const struct config_setting_t *setting = NULL;
	bool retval = true;
...
{
	ShowStatus("Fechando conexao com o Map-Serve....\n");
...
static CPCMD(gm_position)
{
	...
	ShowInfo("HCP: atualizado posicao do console para '"CL_WHITE"%d %d %s"CL_RESET"'\n",x,y,map_name);
	map->cpsd->bl.x = x;
	map->cpsd->bl.y = y;
	map->cpsd->bl.m = m;
}
static CPCMD(gm_use)
{
	...
	if( line == NULL ) {
		ShowError("gm: uso de sintaxe invalida '"CL_WHITE"gm:use @command <parametos opcionais>"CL_RESET"'\n");
		return;
	}
	...
	if( !atcommand->exec(map->cpsd->fd, map->cpsd, line, false) )
		...
		ShowInfo("HCP: '"CL_WHITE"%s"CL_RESET"' em uso\n",line);
...
int do_init(int argc, char *argv[])
{
	...
	if (!minimal) {
		...
		if (!map->ip_set || !map->char_ip_set) {
			...
			if (sockt->naddr_ == 0)
				ShowError("Impossivel identificar o IP...\n");
			else if (sockt->naddr_ > 1)
				ShowNotice("Multiplas interfaces detectadas...\n");

			ShowInfo("Definindo de %s como IP\n", ip_str);
	...
	if (battle_config.pk_mode)
		ShowNotice("Servidor esta em modo '"CL_WHITE"PK"CL_RESET"'.\n");
	...
	ShowStatus("Map Serve '"CL_GREEN"pronto"CL_RESET"' acessando pela porta '"CL_WHITE"%d"CL_RESET"'.\n\n", map->port);
