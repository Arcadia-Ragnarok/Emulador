...
#include "config/core.h" // DBPATH, GP_BOUND_ITEMS, MAX_SPIRITBALL, SECURE_NPCTIMEOUT
#include "pc.h"
...
#else
				clif->messagecolor_self(sd->fd, COLOR_CYAN, msg_txt( 51));
#endif
				return 0;
			}
			break;
...
#else
				clif->messagecolor_self(sd->fd, COLOR_RED, msg_txt( 49));
#endif
				return 0;
			}
			/* Fall through */
...
static int pc_useitem(struct map_session_data *sd, int n)
{
	...
	for(i = 0; i < map->list[sd->bl.m].zone->disabled_items_count; i++) {
		if( map->list[sd->bl.m].zone->disabled_items[i] == nameid ) {
			...
			clif->skill_mapinfomessage(sd, 3);
#else
			clif->messagecolor_self(sd->fd, COLOR_CYAN, msg_txt( 50));
#endif
		...
		if ((t = libconfig->setting_get_member(skt, "skills")) != NULL) {
			...
			while ((sk = libconfig->setting_get_elem(t, j++)) != NULL) {
				...
				if (tree_entry->id != 0 && !tree_entry->inherited) {
					ShowNotice("pc_read_skill_tree: Duplicado %d para '%s' (%d). Ignorado.\n", skill_id, job_name, job_id);
					continue;
				}
...
static bool pc_read_exp_db_sub_class(struct config_setting_t *t, bool base)
{
	...
	if (libconfig->setting_lookup_int(t, "MaxLevel", &maxlv) == 0
		...
		ShowError("pc_read_exp_db_sub_class: Invalid max %s level '%d' set for entry '%s'. Defaulting to %d...", base ? "base" : "job", maxlv, entry.name, 175); // MAX_LEVEL
		maxlv = 175; // MAX_LEVEL
	}
	...
	if ((exp_t = libconfig->setting_lookup(t, "Exp")) != NULL && config_setting_is_array(exp_t)) {
		...
		if (j - 1 < maxlv - 2) {
			...
			ShowInfo("Enchendo os valores perdidos da ultima entrada de exp.\n");
			while (j++ <= maxlv - 2)
...
/**
 * Description: Helper function to read a root configuration in the JobExp.conf file.
...
/**
 * Description: Initiates reading of the JobExp.conf.
...
/*==========================================
 * PC DB reading.
 * JobExp.conf - required experience values
...
static int pc_readdb(void)
{
	...
	/**
	 * Read and load into memory, the JobExp.conf file.
	 */
	pc->clear_exp_groups();
	pc->read_exp_db();
	...
	if(fp == NULL){
		...
		ShowStatus("Leitura de '"CL_WHITE"%u"CL_RESET"' registros em '"CL_WHITE"%s/%s"CL_RESET"'.\n",count,map->db_path,"Status_DB/Statpoint.txt");
	}
