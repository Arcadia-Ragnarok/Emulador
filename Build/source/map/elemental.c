...
static int read_elementaldb(void)
{
	...
	fclose(fp);
	ShowStatus("Leitura de '"CL_WHITE"%d"CL_RESET"' registor em '"CL_WHITE"Database/Summon_DB/Elemental.txt"CL_RESET"'.\n",j);
...
static int read_elemental_skilldb(void)
{
	...
	fp = fopen(line, "r");
	if( !fp ) {
		ShowError("read_elemental_skilldb : can't read ElementSkill.txt\n");
		return -1;
	}
	...
	fclose(fp);
	ShowStatus("Leitura de '"CL_WHITE"%d"CL_RESET"' registros em '"CL_WHITE"Database/Summon_DB/ElementSkill.txt"CL_RESET"'.\n",j);
	return 0;
}
