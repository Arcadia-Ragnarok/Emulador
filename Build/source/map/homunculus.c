...
static void homunculus_read_db(void) {
	const char *filename = "Summon_DB/Homunculus.txt";
	memset(homun->dbs->db, 0, sizeof(homun->dbs->db));
	char filepath[256];
	safesnprintf(filepath, 256, "%s/%s", map->db_path, filename);
	if(!exists(filepath) ) {
		ShowWarning("Arquivo não %s encontrado.\n", filepath);
	} else {
		sv->readdb(map->db_path, filename, ',', 50, 50, MAX_HOMUNCULUS_CLASS, homun->read_db_sub);
	}
}
...
static void homunculus_skill_db_read(void)
{
	...
	sv->readdb(map->db_path, "Summon_DB/HomunSkill.txt", ',', 13, 15, -1, homun->read_skill_db_sub);
}
