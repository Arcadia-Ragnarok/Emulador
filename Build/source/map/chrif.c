...
static bool chrif_setip(const char *ip)
{
	...
	ShowInfo("IP do char-serve : '"CL_WHITE"%s"CL_RESET"' -> '"CL_WHITE"%s"CL_RESET"'.\n", ip, sockt->ip2str(chrif->ip, ip_str));
...
static void chrif_sendmap(int fd)
{
	...
	ShowStatus("Enviando mapas pata o char server...\n");
...
static void chrif_recvmap(int fd)
{
	...
	if (battle_config.etc_log)
		ShowStatus("Recebidos mapas de %u.%u.%u.%u:%u (%d maps)\n", CONVIP(ip), port, j);
...
static void chrif_removemap(int fd)
{
	...
	if(battle_config.etc_log)
		ShowStatus("Removido mapas do server %u.%u.%u.%u:%u (%d maps)\n", CONVIP(ip), port, j);
}
...
static void chrif_connectack(int fd)
{
	...
	ShowStatus("Conexao c Char Server concluida (Conectado: '"CL_WHITE"%d"CL_RESET"').\n",fd);
	chrif->state = 1;
	chrif->connected = 1;

	chrif->sendmap(fd);

	ShowStatus("Evento '"CL_WHITE"OnInterIfInit"CL_RESET"' sendo executado por '"CL_WHITE"%d"CL_RESET"' NPCs.\n", npc->event_doall("OnInterIfInit"));
	if( !char_init_done ) {
		char_init_done = true;
		ShowStatus("Evento '"CL_WHITE"OnInterIfInitOnce"CL_RESET"' sendo executado por '"CL_WHITE"%d"CL_RESET"' NPCs.\n", npc->event_doall("OnInterIfInitOnce"));
...
static void chrif_on_ready(void)
{
	...
	ShowStatus("Map Server esta online.\n");
	...
	if(DIFF_TICK(timer->gettick(),node->node_created)>60000) {
		...
		switch (node->state) {
			...
			default:
				...
				ShowInfo("auth_db: (estado %s) tempo esgotado para %d:%d\n", states[node->state], node->account_id, node->char_id);
...
static bool chrif_changeemail(int id, const char *actual_email, const char *new_email)
{
	...
	if (battle_config.etc_log)
		ShowInfo("chrif_changeemail: conta: %d, atual email: '%s', novo email: '%s'.\n", id, actual_email, new_email);
...
static void chrif_idbanned(int fd)
{
	...
	if (RFIFOB(fd,6) == 0) { // 0: change of status
		...
		if(0<ret_status && ret_status<=9)
			...
			clif->message(sd->fd, msg_txt(421)); // Your account has been totally erased.
		else
			clif->message(sd->fd, msg_txt(420)); //"Your account is not longer authorized."
...
static void chrif_recvfamelist(int fd)
{
	...
	ShowInfo("Recebido lista de fama de '"CL_WHITE"%d"CL_RESET"' personagens.\n", total);
}
...
static int check_connect_char_server(int tid, int64 tick, int id, intptr_t data)
{
	...
	if ( chrif->fd <= 0 || sockt->session[chrif->fd] == NULL ) {
		if ( !displayed ) {
			ShowStatus("Tentando se conectar ao Char Server.\n");
			displayed = 1;
		}
