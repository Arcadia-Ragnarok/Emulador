...
static void login_fromchar_parse_auth(int fd, int id, const char *const ip)
{
	...
	{// authentication not found
		nullpo_retv(ip);
		ShowStatus("Char-server '%s': autentificacao da conta %d RECUSADA (ip: %s).\n", login->dbs->server[id].name, account_id, ip);
...
static void login_fromchar_parse_request_change_email(int fd, int id, const char *const ip)
{
	...
	if( e_mail_check(email) == 0 )
		ShowNotice("Char-server '%s': Tentativa de criar e-mail falso - RECUSADO (conta: %d, ip: %s)\n", login->dbs->server[id].name, account_id, ip);
	else
	if( !accounts->load_num(accounts, &acc, account_id) || strcmp(acc.email, "a@a.com") == 0 || acc.email[0] == '\0' )
		ShowNotice("Char-server '%s': Tentativa de criar e-mail falso - RECUSADO - A conta nao exite ou mesmo o e-mail e invalido (conta: %d, ip: %s).\n", login->dbs->server[id].name, account_id, ip);
	else {
		memcpy(acc.email, email, sizeof(acc.email));
		ShowNotice("Char-server '%s': Creado e-mail padrao (account: %d, novo e-mail: %s, ip: %s).\n", login->dbs->server[id].name, account_id, email, ip);
		// Save
...
static void login_fromchar_parse_change_email(int fd, int id, const char *const ip)
{
	...
	if( e_mail_check(actual_email) == 0 )
		ShowNotice("Char-server '%s': Tentativa de modificar e-mail da conta com dados invalidos (conta: %d, ip: %s)\n", login->dbs->server[id].name, account_id, ip);
	else
	...
	if( strcmpi(acc.email, actual_email) != 0 )
		ShowNotice("Char-server '%s': Tentativa de modificar e-mail da conta com dados invalidos (conta: %d (%s), actual e-mail: %s, proposed e-mail: %s, ip: %s).\n", login->dbs->server[id].name, account_id, acc.userid, acc.email, actual_email, ip);
	else {
		safestrncpy(acc.email, new_email, sizeof(acc.email));
		ShowNotice("Char-server '%s': Tentativa de modificar e-mail da conta com dados invalidos (conta: %d (%s), new e-mail: %s, ip: %s).\n", login->dbs->server[id].name, account_id, acc.userid, new_email, ip);
		// Save
		accounts->save(accounts, &acc);
	}
}
...
static void login_fromchar_parse_account_update(int fd, int id, const char *const ip)
{
	...
	if( !accounts->load_num(accounts, &acc, account_id) )
		ShowNotice("Char-server '%s': Erro ao atualizar (conta: %d nao encontrada, sugerido %u, ip: %s).\n", login->dbs->server[id].name, account_id, state, ip);
	else
	if( acc.state == state )
		ShowNotice("Char-server '%s':  Erro ao atualizar - estado atual ja esta bem (conta: %d, estado %u, ip: %s).\n", login->dbs->server[id].name, account_id, state, ip);
	else {
		ShowNotice("Char-server '%s': Estado alterado (conra: %d, novo %u, ip: %s).\n", login->dbs->server[id].name, account_id, state, ip);
...
static void login_fromchar_parse_ban(int fd, int id, const char *const ip)
{
	...
	if (!accounts->load_num(accounts, &acc, account_id)) {
		ShowNotice("Char-server '%s': Erro banir (conta: %d nao enconrada, ip: %s).\n", login->dbs->server[id].name, account_id, ip);
	} else {
		time_t timestamp;
		struct tm *tmtime;
		...
		if (timestamp == -1) {
			ShowNotice("Char-server '%s': Erro ao banir (conta: %d, data invalida, ip: %s).\n", login->dbs->server[id].name, account_id, ip);
		} else if( timestamp <= time(NULL) || timestamp == 0 ) {
			ShowNotice("Char-server '%s': Erro ao banir (conta: %d, pela data de desbanimento, ip: %s).\n", login->dbs->server[id].name, account_id, ip);
		} else {
			char tmpstr[24];
			timestamp2string(tmpstr, sizeof(tmpstr), timestamp, login->config->date_format);
			ShowNotice("Char-server '%s': Banimento (conta: %d, data de termino: %ld (%s), ip: %s).\n",
...
static void login_fromchar_parse_change_sex(int fd, int id, const char *const ip)
{
	...
	if( !accounts->load_num(accounts, &acc, account_id) )
		ShowNotice("Char-server '%s': Erro ao alterar sexo (conta: %d nao encontrada, ip: %s).\n", login->dbs->server[id].name, account_id, ip);
	else
	if( acc.sex == 'S' )
		ShowNotice("Char-server '%s': Erro ao alterar sexo - essa e uma conta de Servidor (conta: %d, ip: %s).\n", login->dbs->server[id].name, account_id, ip);
	else
	{
		char sex = ( acc.sex == 'M' ) ? 'F' : 'M'; //Change gender

		ShowNotice("Char-server '%s': Sexo alterado (conta: %d, novo %c, ip: %s).\n", login->dbs->server[id].name, account_id, sex, ip);
...
static void login_fromchar_parse_account_reg2(int fd, int id, const char *const ip)
{
	...
	if( !accounts->load_num(accounts, &acc, account_id) )
		ShowStatus("Char-server '%s': recebeu (do char-server) de account_reg2 (conta: %d nao encontrado, ip: %s).\n", login->dbs->server[id].name, account_id, ip);
	else {
...
static void login_fromchar_parse_unban(int fd, int id, const char *const ip)
{
	...
	if( !accounts->load_num(accounts, &acc, account_id) )
		ShowNotice("Char-server '%s': Erro ao desbanir (conta: %d nao encontrada, ip: %s).\n", login->dbs->server[id].name, account_id, ip);
	else
	if( acc.unban_time == 0 )
		ShowNotice("Char-server '%s': Erro ao desbanir (conta: %d, nenhuma data, ip: %s).\n", login->dbs->server[id].name, account_id, ip);
	else
	{
		ShowNotice("Char-server '%s': Desbanimento (conta: %d, ip: %s).\n", login->dbs->server[id].name, account_id, ip);
		acc.unban_time = 0;
		accounts->save(accounts, &acc);
	}
}
...
static void login_fromchar_parse_update_wan_ip(int fd, int id)
{
	...
	ShowInfo("Atualizado IP #%d para %u.%u.%u.%u.\n",id, CONVIP(login->dbs->server[id].ip));
	RFIFOSKIP(fd,6);
}

static void login_fromchar_parse_all_offline(int fd, int id)
{
	ShowInfo("definindo contas para o char-server %d como offline.\n", id);
...
static int login_mmo_auth_new(const char *userid, const char *pass, const char sex, const char *last_ip)
{
	...
	if (DIFF_TICK(tick, new_reg_tick) < 0 && num_regs >= login->config->allowed_regs) {
		ShowNotice("Registro de conta negado (LIMITE EXEDIDO)\n");
		return 3;
	}
	...
	if( accounts->load_str(accounts, &acc, userid) ) {
		ShowNotice("Tentativa de criar uma conta ja existente (conta: %s_%c, senha: %s, recebeu como senha: %s)\n", userid, sex, acc.pass, pass);
...
static int login_mmo_auth(struct login_session_data *sd, bool isServer)
{
	...
	if (login->config->use_dnsbl) {
		...
		for (i = 0; i < VECTOR_LENGTH(login->config->dnsbl_servers); i++) {
			...
			if (sockt->host2ip(ip_dnsbl)) {
				ShowInfo("DNSBL: (%s) Lista negra. Usuario expulso.\n", r_ip);
				return 3;
			}
		}
	...
	if( len <= 0 ) { /** a empty password is fine, a userid is not. **/
		ShowNotice("userid vazio (senha: '%s', ip: %s)\n", sd->passwd, ip);
		return 0; // 0 = Unregistered ID
	}

	if( !accounts->load_str(accounts, &acc, sd->userid) ) {
		ShowNotice("conta desconhecida (conta: %s, susposta senha: %s, ip: %s)\n", sd->userid, sd->passwd, ip);
		return 0; // 0 = Unregistered ID
	}

	if( !login->check_password(sd->md5key, sd->passwdenc, sd->passwd, acc.pass) ) {
		ShowNotice("Senha invalida (conta: '%s', senha: '%s', susposta senha: '%s', ip: %s)\n", sd->userid, acc.pass, sd->passwd, ip);
		return 1; // 1 = Incorrect Password
	}
	...
	if( acc.unban_time != 0 && acc.unban_time > time(NULL) ) {
		...
		ShowNotice("Conexao recusada (conta: %s, senha: %s, Banimento util %s, ip: %s)\n", sd->userid, sd->passwd, tmpstr, ip);
		return 6; // 6 = Your are Prohibited to log in until %s
	}

	if( acc.state != 0 ) {
		ShowNotice("Conexao recusada (conta: %s, senha: %s, estado: %u, ip: %s)\n", sd->userid, sd->passwd, acc.state, ip);
		return acc.state - 1;
	}
	...
	if (login->config->client_hash_check && !isServer) {
		...
		if( !match ) {
			...
			if( !sd->has_client_hash ) {
				ShowNotice("O cliente nao pode enviar seu hash (conta: %s, senha: %s, ip: %s)\n", sd->userid, sd->passwd, ip);
				return 5;
			}
			...
			ShowNotice("Hash do cliente invalido (conta: %s, senha: %s, md5: %s, ip: %s)\n", sd->userid, sd->passwd, smd5, ip);
			return 5;
		}
	}

	ShowNotice("Autentificacao aceita (conta: %s, id: %d, ip: %s)\n", sd->userid, acc.account_id, ip);
...
static void login_auth_ok(struct login_session_data *sd)
{
	...
	if (login->config->group_id_to_connect >= 0 && sd->group_id != login->config->group_id_to_connect) {
		ShowStatus("Conexao recusada: requer GROUPID %d para conectar (conta: %s, group: %d).\n", login->config->group_id_to_connect, sd->userid, sd->group_id);
		lclif->connection_error(fd, 1); // 01 = server closed
		return;
		...
		ShowStatus("Conexao recusada: minimo GROUPID para conectar %d (conta: %s, group: %d).\n", login->config->min_group_id_to_connect, sd->userid, sd->group_id);
		lclif->connection_error(fd, 1); // 01 = server closed
		return;
	}

	{
		...
		{// account is already marked as online!
			...
			{// Request char servers to kick this account out. [Skotlex]
				ShowNotice("Usuario '%s' esta online - rejeitado.\n", sd->userid);
				login->kick(sd);
	...
	if (!lclif->server_list(sd)) {
		...
		ShowStatus("Conexao recusada: o char-server nao esta online (conta: %s).\n", sd->userid);
		lclif->connection_error(fd, 1); // 01 = server closed
		return;
	}
...
static void login_parse_request_connection(int fd, struct login_session_data* sd, const char *const ip, uint32 ipl)
{
	...
	ShowInfo("Conexao com char-server '%s' @ %u.%u.%u.%u:%u (conta: '%s', senha: '%s', ip: '%s')\n", server_name, CONVIP(server_ip), server_port, sd->userid, sd->passwd, ip);
	...
	{
		ShowStatus("Conexao com char-server '%s' aceita.\n", server_name);
...
/**
 * Reads 'login_configuration/inter_connection' and initializes required variables.
 *
 ...
 */
static bool login_config_read_inter(const char *filename, struct config_t *config)
{
	...
	if ((setting = libconfig->lookup(config, "login_configuration/inter_connection")) == NULL) {
		ShowError("login_config_read: login_configuration/inter_connection was not found in %s!\n", filename);
		return false;
	}
	...
	if (libconfig->setting_lookup_string(setting, "bind_ip", &str) == CONFIG_TRUE) {
		...
		if ((login->config->login_ip = sockt->host2ip(str)) != 0)
			ShowStatus("Login server ligando endereco de IP : %s -> %s\n", old_ip_str, str);
	}
 ...
 * @retval false in case of error.
 */
static bool login_config_read_console(const char *filename, struct config_t *config)
{
	struct config_setting_t *setting = NULL;
	...
	if (libconfig->setting_lookup_int(setting, "console_silent", &showmsg->silent) == CONFIG_TRUE) {
		if (showmsg->silent) // only bother if its actually enabled
			ShowInfo("Silenciano o console: %d\n", showmsg->silent);
	}
 ...
 * @retval false in case of error.
 */
static bool login_config_read_log(const char *filename, struct config_t *config)
{
	struct config_setting_t *setting = NULL;
 ...
 * @retval false in case of error.
 */
static bool login_config_read_account(const char *filename, struct config_t *config)
{
	struct config_setting_t *setting = NULL;
	...
	if (!db->set_property(db, config))
		retval = false;
	if (!ipban->config_read(filename, config))
		retval = false;
 ...
 * @retval false in case of error.
 */
static bool login_config_read_permission_hash(const char *filename, struct config_t *config)
{
	struct config_setting_t *setting = NULL;
 ...
 * @retval false in case of error.
 */
static bool login_config_read_permission_blacklist(const char *filename, struct config_t *config)
{
	struct config_setting_t *setting = NULL;
 ...
 * @retval false in case of error.
 */
static bool login_config_read_permission(const char *filename, struct config_t *config)
{
	struct config_setting_t *setting = NULL;
	bool retval = true;
	...
	if (!login->config_read_permission_hash(filename, config))
		retval = false;
	if (!login->config_read_permission_blacklist(filename, config))
		retval = false;
 ...
 * @retval false in case of error.
 */
static bool login_config_read_users(const char *filename, struct config_t *config)
{
	struct config_setting_t *setting = NULL;
	bool retval = true;
 ...
 * @retval false in case of error.
 **/
static bool login_config_read(const char *filename)
{
	...
	if (!login->config_read_inter(filename, &config))
		retval = false;
	if (!login->config_read_console(filename, &config))
		retval = false;
	if (!login->config_read_log(filename, &config))
		retval = false;
	if (!login->config_read_account(filename, &config))
		retval = false;
	if (!login->config_read_permission(filename, &config))
		retval = false;
	if (!login->config_read_users(filename, &config))
		retval = false;

	if (!loginlog->config_read("Config/Servers/Inter-Server.cs")) // Only inter-server
		retval = false;

	ShowInfo("Finaliza leitura %s.\n", filename);
...
int do_init(int argc, char **argv)
{
	...
	login->config_read(login->LOGIN_CONF_NAME);
	...
	ShowStatus("login-server "CL_GREEN"pronto"CL_RESET" (acessando pela porta %u).\n\n", login->config->login_port);
