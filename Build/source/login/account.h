...
struct AccountDB
{
	...
	/// @return true if successful
	bool (*set_property)(AccountDB* self, struct config_t *config);
...
struct account_interface {
	...
	bool (*db_sql_set_property) (AccountDB* self, struct config_t *config);
	...
	bool (*db_read_inter) (AccountDB_SQL *db, const char *filename);
};
