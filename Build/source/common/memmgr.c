...
#ifdef LOG_MEMMGR
	if(count == 0) {
		ShowInfo("Gerenciamento de memoria: Nenhum vazamento.\n");
	} else {
		ShowWarning("Gerenciamento de memoria: Vazamento detectado e estabilizado.\n");
		fclose(log_fp);
	}
#endif /* LOG_MEMMGR */
}
...
static void memmgr_init_messages(void)
{
	...
	ShowStatus("Gerenciamento de memoria iniciado: "CL_WHITE"%s"CL_RESET"\n", memmer_logfile);
#endif /* LOG_MEMMGR */
}
#endif /* USE_MEMMGR */
