...
static bool usercheck(void) {
	ShowMessage("\n");
	ShowNotice("Emulador sendo executado com privilegios de root.\n");
	return true;
}
 ...
 */
static bool cmdline_arg_add(const char *name, char shortname, CmdlineExecFunc func, const char *help, unsigned int options)
{
...
static CMDLINEARG(help)
{
	...
	for (i = 0; i < VECTOR_LENGTH(cmdline->args_data); i++) {
		...
		ShowInfo("  %-30s %s [%s]\n", paramnames, data->help ? data->help : "<nenhuma descricao>", cmdline->arg_source(data));
	}
	return false;
}
