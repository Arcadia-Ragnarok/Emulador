	...
	for(i = 1; i < sockt->fd_max; i++)
	{
		...
		if (sockt->session[i]->rdata_tick && DIFF_TICK(sockt->last_tick, sockt->session[i]->rdata_tick) > sockt->stall_time) {
			if( sockt->session[i]->flag.server ) {/* server is special */
				...
				ShowInfo("Sessao #%d esgotou o tempo\n", i);
				sockt->eof(i);
			}
		}
...
static int connect_check_(uint32 ip)
{
	...
	for (i = 0; i < VECTOR_LENGTH(access_allow); ++i) {
		...
		if (SUBNET_MATCH(ip, entry->ip, entry->mask)) {
			if (access_debug) {
				ShowInfo("connect_check: Lista:%u.%u.%u.%u IP:%u.%u.%u.%u Mask:%u.%u.%u.%u\n",
					CONVIP(ip),
					CONVIP(entry->ip),
					CONVIP(entry->mask));
			}
	...
	for (i = 0; i < VECTOR_LENGTH(access_deny); ++i) {
		...
		if (SUBNET_MATCH(ip, entry->ip, entry->mask)) {
			if (access_debug) {
				ShowInfo("connect_check: Lista negada:%u.%u.%u.%u IP:%u.%u.%u.%u Mask:%u.%u.%u.%u\n",
					CONVIP(ip),
					CONVIP(entry->ip),
					CONVIP(entry->mask));
			}
...
static int connect_check_clear(int tid, int64 tick, int id, intptr_t data)
{
	...
	if( access_debug ){
		ShowInfo("connect_check_clear: Limpado %d de %d da lista de IP.\n", clear, list);
	}
...
static int access_ipmask(const char *str, struct access_control *acc)
{
	...
	if( access_debug ){
		ShowInfo("access_ipmask: Cerredado IP:%u.%u.%u.%u mask:%u.%u.%u.%u\n", CONVIP(ip), CONVIP(mask));
	}
	acc->ip   = ip;
	acc->mask = mask;
	return 1;
 ...
 */
static bool socket_config_read_iprules(const char *filename, struct config_t *config)
{
 ...
 * @retval false in case of error.
 */
static bool socket_config_read_ddos(const char *filename, struct config_t *config)
{
	struct config_setting_t *setting = NULL;
 ...
 * @retval false in case of error.
 */
static bool socket_config_read(const char *filename)
{
...
static void socket_init(void)
{
	...
	socket_config_read(sockt->SOCKET_CONF_FILENAME);

#ifndef SOCKET_EPOLL
	// Select based Event Dispatcher:
	sFD_ZERO(&readfds);
	//ShowInfo("Usuarios '" CL_WHITE "selecionados" CL_RESET "' como despachante de evento\n");

#else  // SOCKET_EPOLL
	// Epoll based Event Dispatcher:
	epfd = epoll_create(FD_SETSIZE); // 2.6.8 or newer ignores the expected socket amount argument
	if(epfd == SOCKET_ERROR){
		ShowError("Nao criou o Epoll Evento Despachante: %s\n", error_msg());
		exit(EXIT_FAILURE);
	}
	...
	ShowInfo("Salvo usuarios '" CL_WHITE "epoll" CL_RESET "' com ate " CL_WHITE "%d" CL_RESET " eventos por ciclo\n", epoll_maxevents);
	...
	ShowInfo("Servidor esta suportando '"CL_WHITE"%"PRIu64""CL_RESET"' conexoes simultaneas.\n", rlim_cur);
}
...
/**
 * Helper function to read a list of NetWork.cs values.
 *
...
*/
	...
	if (sockt->net_config_read_sub(libconfig->lookup(&network_config, "allowed"), &sockt->allowed_ips, filename, "allowed") > 0)
		ShowStatus("Lendo informacao sobre IP %d como permitido.\n", (int)VECTOR_LENGTH(sockt->allowed_ips));
...
#ifndef BUILDBOT
	if (i != VECTOR_LENGTH(sockt->allowed_ips)) {
		ShowInfo("Nenhum sufixo de IP e DNS configurado, todas conexoes serao aceitas.\n");
	}
#endif  // BUILDBOT
...
void socket_defaults(void)
{
	...
	sockt->SOCKET_CONF_FILENAME = "Config/System/Socket.cs";
